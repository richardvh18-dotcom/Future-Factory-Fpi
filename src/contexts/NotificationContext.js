...
const inferAlertType = (message) => {
    ...
    if (/^(❌|error|fout|mislukt|kon .* niet|kan .* niet)/iu.test(value))
        return 'error';
    if (/^(⚠️|waarschuwing|let op)/iu.test(value))
        return 'warning';
    if (/^(✅|succes|gelukt)/iu.test(value))
        return 'success';
    return 'info';
};
...
export const NotificationProvider = ({ children }) => {
    ...
    const [hasPermission, setHasPermission] = useState(typeof window !== 'undefined' && 'Notification' in window
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (window.__APP_ALERT__)
                delete window.__APP_ALERT__;
            if (originalAlertRef.current) {
    ...
    useEffect(() => {
        ...
        const q = query(messagesRef, where('to', 'in', [user.email.toLowerCase(), 'admin']));
