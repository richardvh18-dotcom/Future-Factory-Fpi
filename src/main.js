...
const reportRuntimeError = (type, details) => {
    try {
        ...
        logActivity(auth.currentUser?.uid || "system", "CLIENT_RUNTIME_ERROR", `${type}: ${String(details || "unknown").slice(0, 1200)}`).catch(() => { });
    }
    catch {
...
if (!rootElement) {
    ...
    console.error("CRITICAL ERROR: Het element met id 'root' is niet gevonden in de HTML.");
}
else {
    const root = ReactDOM.createRoot(rootElement);
    root.render(_jsx(React.StrictMode, { children: _jsx(ErrorBoundary, { children: _jsx(BrowserRouter, { future: {
...
window.addEventListener('error', (event) => {
    ...
    reportRuntimeError('window.error', `${event.message || 'unknown'} @ ${event.filename || 'unknown'}:${event.lineno || 0}`);
});
