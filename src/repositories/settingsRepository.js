...
export const fetchGeneralConfig = async () => {
    if (!isValidPath('GENERAL_SETTINGS'))
        return {};
...
export const fetchAllSettings = async () => {
    const [generalConfig, productRange, boreDimensions, cbDimensions, tbDimensions,] = await Promise.all([
        fetchGeneralConfig(),
        fetchMatrixConfig(),
...
export const fetchLabelTemplates = async () => {
    if (!isValidPath('LABEL_TEMPLATES'))
        return [];
