...
const pickBestDateCandidate = (candidates, expectedWeekNumber = null) => {
    ...
    const ranked = candidates
        ...
        .map((candidate, index) => {
        let score = 0;
        if (candidate.priority === "preferred")
            score += 10;
        if (hasExpectedWeek) {
...
const processDates = (rawDate, expectedWeekNumber = null) => {
    if (!rawDate)
        return { delivery: null, planned: null };
...
const getSheetPriority = (sheetName) => {
    ...
    if (s.includes("format fabriek"))
        return 100;
    if (s.includes("format mazak"))
        return 90;
    if (s.includes("format 40bm01"))
        return 80;
    if (s === "pasteddata")
        return 75;
    if (s.includes("fabrieksplanning"))
        return 70;
    if (s.includes("mazakplanning"))
        return 60;
    if (s === "40bm01")
        return 50;
    return 10;
...
const parseWorkbook = (arrayBuffer) => {
    ...
    const isAllowed = (name) => ALLOWED_SHEETS.some((a) => name.trim().toLowerCase() === a.toLowerCase());
    for (const sheetName of sheetNames) {
        if (!isAllowed(sheetName))
            continue;
        ...
        const sourceSheetLabel = sheetName.toLowerCase().includes("format") && filterMachine
        ...
        if (headerIndex === -1)
            continue; // Geen planningheader → sheet overslaan
        ...
        const sheetData = dataRows
            ...
            let gewikkeldCount = typeof rawGewikkeld === "string"
                ? parseFloat(rawGewikkeld.replace(",", "."))
                : parseFloat(rawGewikkeld);
            if (Number.isNaN(gewikkeldCount))
                gewikkeldCount = 0;
