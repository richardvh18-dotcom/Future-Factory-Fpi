...
import { isOpenOrRunningOrder, getOrderRemainingQueueQty, getDeliveredQtyForOrder, getInspectionApprovedQtyForOrder, getDeliveryInspectionDeltaForOrder, isEventInCurrentWeek, getLegacyRejectedOrders, buildOverproductionGroups, } from "../../utils/teamleaderDerived";
import { normalizeMachine } from "../../utils/hubHelpers.tsx";
import { getLotFromTrackedRecord, resolveOverproductionRoute, getPriorityLevel, getOverproductionTargetCandidates, } from "./teamleaderHub.helpers";
 ...
 */
const TeamleaderHub = React.memo(({ onBack, onExit, fixedScope = "all", departmentName = "Algemeen", allowedMachines = [], title = "Teamleader Hub", }) => {
    ...
    useEffect(() => {
        if (!exportTrackingTaskId || showExportModal)
            return;
    ...
    const { rawOrders, rawProducts, bezetting, archivedHistoryProducts, archivedRejectedProducts, factoryConfig, loading, dbError, } = useTeamleaderFirestore({ user });
    ...
    const { safeScope, targetSlug, effectiveStations, effectiveAllowedNorms, orderProgressMeta, dataStore, } = useTeamleaderDataStore({
        rawOrders,
        rawProducts,
        factoryConfig,
        fixedScope,
    ...
    const hasActiveTrackingForOrder = (orderId) => {
        ...
        if (!normalizedOrderId)
            return true;
        const relatedProducts = rawProducts.filter((product) => getOrderIdFromTrackedRecord(product) === normalizedOrderId);
    ...
    const isPriorityOrder = (order) => {
        if (getPriorityLevel(order) === "normal")
            return false;
        if (!isOpenOrRunningOrder(order))
            return false;
    ...
    const { handleOpenExtendedPersonnel, handleOpenOverproductionGroup, handleAssignOverproduction, handleSidebarSelect, handleOpenArchivedLotDossier, handleReopenArchivedOrderWithIncrease, handleKpiClick, handleDrawingSync, handleExport, handlePlannerExcelExport, handleCopyYesterday, handleClearToday, handleMoveLot, handleArchiveRejectedProduct, handleCreateOrder, handleArchiveLegacyRejectedOrders, } = useTeamleaderEventHandlers({
        user,
        navigate,
        t,
        todayStr,
    ...
    if (loading)
        return (_jsxs("div", { className: "flex h-full flex-col items-center justify-center bg-slate-50 gap-4", children: [_jsx(Loader2, { className: "animate-spin text-blue-600", size: 48 }), _jsx("p", { className: "text-[10px] font-black uppercase tracking-widest text-slate-400 italic", children: t('teamleader.loading_data', 'Productiedata synchroniseren...') })] }));
    if (!user?.role || user?.role === 'guest')
        return (_jsxs("div", { className: "h-full flex flex-col items-center justify-center p-10 text-center", children: [_jsx("h3", { className: "text-xl font-black uppercase italic text-slate-400", children: t('teamleader.access_denied', 'Toegang Beperkt') }), _jsx("p", { className: "text-slate-500 text-sm mt-2 max-w-xs", children: t('teamleader.no_rights', 'Uw account heeft nog geen rechten om deze data te bekijken.') }), _jsx("button", { onClick: onBack || onExit, className: "mt-8 px-8 py-3 bg-slate-100 text-slate-600 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-slate-200 transition-all", children: t('common.back', 'Terug') })] }));
    if (dbError)
        return (_jsxs("div", { className: "h-full flex flex-col items-center justify-center p-10 text-center", children: [_jsx(AlertTriangle, { size: 48, className: "text-rose-500 mb-4" }), _jsx("h3", { className: "text-xl font-black uppercase italic", children: t('teamleader.db_error_title', 'Database Verbindingsfout') }), _jsx("p", { className: "text-slate-500 text-sm mt-2 max-w-xs", children: t('teamleader.db_error_desc', 'De app kon geen verbinding maken met Firestore (Fout: {{error}}).', { error: dbError }) }), _jsx("button", { onClick: () => window.location.reload(), className: "mt-8 px-8 py-3 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl", children: t('teamleader.retry', 'Opnieuw Proberen') })] }));
    return (_jsx(TeamleaderModalProvider, { value: modalContextValue, children: _jsxs("div", { className: "flex flex-col h-full bg-slate-50 text-left w-full animate-in fade-in duration-300 overflow-hidden relative", children: [_jsx(TeamleaderHeader, { onBack: onBack, onExit: onExit, fixedScope: fixedScope, departmentName: departmentName, title: title, departmentFilter: departmentFilter, setDepartmentFilter: setDepartmentFilter, activeTab: activeTab, setActiveTab: setActiveTab, canManageOverproduction: canManageOverproduction, overproductionGroups: overproductionGroups, isMobileMenuOpen: isMobileMenuOpen, setIsMobileMenuOpen: setIsMobileMenuOpen, showAiPrediction: showAiPrediction, setShowAiPrediction: setShowAiPrediction, isSyncingDrawings: isSyncingDrawings, handleDrawingSync: handleDrawingSync, t: t }), _jsx("div", { className: "flex-1 overflow-hidden p-6 w-full flex flex-col text-left", children: _jsx("div", { className: "flex-1 overflow-y-auto custom-scrollbar relative", children: activeTab === "dashboard" ? (_jsx(TeamleaderDashboard, { metrics: metrics, onKpiClick: handleKpiClick, onStationSelect: setSelectedStationDetail })) : activeTab === "bezetting" ? (_jsxs("div", { className: "space-y-3", children: [_jsx("div", { className: "flex items-center justify-end", children: _jsxs("button", { onClick: handleOpenExtendedPersonnel, className: "px-4 py-2 bg-white border border-indigo-200 text-indigo-700 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all flex items-center gap-2 shadow-sm", title: t('teamleader.open_extended_personnel_admin', 'Open uitgebreide Personeel & Bezetting in Admin Hub'), children: [_jsx(Link2, { size: 14 }), " ", t('teamleader.extended_personnel_module', 'Uitgebreide Personeel Module')] }) }), _jsx(PersonnelOccupancyView, { scope: departmentFilter !== "ALL" ? departmentFilter.toLowerCase() : fixedScope, onCopyYesterday: handleCopyYesterday, isCopying: isCopying, onClearToday: handleClearToday, isClearing: isClearing })] })) : activeTab === "efficiency" ? (showAiPrediction ? (_jsx(AiPredictionView, { onClose: () => setShowAiPrediction(false) })) : (_jsx(TeamleaderEfficiencyView, { departmentName: departmentFilter !== "ALL" ? departmentFilter : departmentName, lockDepartment: fixedScope !== "all" }))) : activeTab === "import_export" ? (_jsx(ImportExportDashboard, { currentDepartment: departmentFilter !== "ALL" ? departmentFilter.toLowerCase() : targetSlug, onCreateOrder: () => setShowAddOrderModal(true), trackedProducts: rawProducts, archivedHistoryProducts: archivedHistoryProducts, effectiveAllowedNorms: effectiveAllowedNorms, planningOrders: dataStore, onOpenMachineExport: (type) => {
                                setExportModalType(type || "planning");
                                setExportModalLocked(true);
                                setShowExportModal(true);
                            } })) : (_jsx(TeamleaderSelectionProvider, { value: selectionContextValue, children: _jsxs("div", { className: "h-full flex flex-col overflow-hidden", children: [_jsx(SmartPlanningSuggestions, { orders: dataStore, onOrderClick: handleSidebarSelect }), _jsxs("div", { className: "flex-1 flex gap-6 min-h-0 overflow-hidden", children: [_jsx(TeamleaderOrderRail, { canManageOverproduction: canManageOverproduction, overproductionGroups: overproductionGroups, onOpenOverproductionGroup: handleOpenOverproductionGroup, resolveOverproductionRoute: resolveOverproductionRoute, orders: dataStore, trackedProducts: rawProducts, archivedHistoryProducts: archivedHistoryProducts }), _jsx(TeamleaderDetailPane, { handleMoveLot: handleMoveLot, setViewingDossier: setViewingDossier, targetSlug: targetSlug, effectiveStations: effectiveStations, rawProducts: rawProducts, archivedHistoryProducts: archivedHistoryProducts, handleOpenArchivedLotDossier: handleOpenArchivedLotDossier, handleReopenArchivedOrderWithIncrease: handleReopenArchivedOrderWithIncrease })] })] }) })) }) }), _jsx(TeamleaderModals, {}), showExportModal && (_jsx(TeamleaderExportModal, { isOpen: showExportModal, onClose: () => {
                        setShowExportModal(false);
                        setExportModalLocked(false);
                        setExportPreloadedTask(null);
                    }, rawOrders: rawOrders, rawProducts: rawProducts, archivedProducts: archivedHistoryProducts, initialExportType: exportModalType, lockExportType: exportModalLocked, onTaskCreated: (taskId) => {
