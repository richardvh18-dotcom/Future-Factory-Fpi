 ...
 */
export const useTeamleaderDataStore = ({ rawOrders, rawProducts, factoryConfig, fixedScope, allowedMachines, departmentFilter, getOrderIdFromTrackedRecord, getLotFromTrackedRecord, }) => {
    ...
    const effectiveStations = useMemo(() => {
        ...
        if (factoryConfig && factoryConfig.departments && safeScope !== "all") {
            const dept = factoryConfig.departments.find((d) => d.slug === targetSlug ||
                d.id === targetSlug ||
            ...
            if (departmentFilter !== "ALL") {
                ...
                const dept = factoryConfig.departments.find((d) => d.slug === filterSlug ||
                    d.id === filterSlug ||
    ...
    const effectiveAllowedNorms = useMemo(() => {
        ...
        if (safeScope === "fittings") {
            const fittingNorms = FITTING_MACHINES.map((stationName) => normalizeMachine(stationName)).filter(Boolean);
    ...
    const orderProgressMeta = useMemo(() => {
        ...
        rawProducts.forEach((product) => {
            ...
            const inScope = effectiveAllowedNorms.length === 0 ||
                [machineNorm, originNorm, currentNorm, lastNorm].some((value) => value ? effectiveAllowedNorms.includes(value) : false);
            if (!inScope)
                return;
    ...
    const dataStore = useMemo(() => {
        ...
        return rawOrders
            ...
            const shouldShowMazakStatus = isMazakMachine &&
                [
                    "te_nabewerken",
            ...
            if (targetSlug === "all" && departmentFilter === "ALL")
                return true;
            if (effectiveAllowedNorms.length > 0) {
                ...
                if (hasStartedInScope)
                    return true;
                if ((progressMeta?.trackedInScopeCount || 0) > 0)
                    return true;
