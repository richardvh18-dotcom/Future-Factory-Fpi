...
export const useTeamleaderFirestore = ({ user }) => {
    ...
    useEffect(() => {
        ...
        const initData = async () => {
            ...
            const unsubRootOrders = onSnapshot(collection(db, ...PATHS.PLANNING), (snap) => {
            ...
            const unsubOcc = onSnapshot(collection(db, ...PATHS.OCCUPANCY), (snap) => {
            ...
            const syncArchiveHistory = () => {
                ...
                const combined = Object.values(archiveDataByYear)
                    .flatMap((items) => items || [])
                    .sort((a, b) => {
                    const aMs = a?.timestamps?.finished?.toMillis?.() ||
                        a?.updatedAt?.toMillis?.() ||
                        new Date(a?.updatedAt || 0).getTime() ||
                        0;
                    const bMs = b?.timestamps?.finished?.toMillis?.() ||
            ...
            [now.getFullYear(), now.getFullYear() - 1].forEach((historyYear) => {
                const unsubArchiveYear = onSnapshot(query(collection(db, ...getArchiveItemsPath(historyYear)), where("timestamps.finished", ">=", minArchiveDate)), (snap) => {
                    if (!isMounted)
                        return;
                ...
                const unsubRejectedYear = onSnapshot(collection(db, ...getArchiveRejectedItemsPath(historyYear)), (snap) => {
                    if (!isMounted)
                        return;
