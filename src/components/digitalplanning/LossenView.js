...
import { Package, Loader2, History, ArrowRight, ChevronDown, ChevronRight, ScanBarcode, Keyboard } from "lucide-react";
...
const hasOriginInSet = (candidates, stationSet) => candidates.some((value) => stationSet.has(normalizeMachine(value || "")));
const shouldBelongToLossen1218 = (item, candidates) => {
    if (hasOriginInSet(candidates, LOSSEN_1218_DIRECT_STATIONS))
        return true;
    if (hasOriginInSet(candidates, LOSSEN_1218_BH18_STATIONS))
        return !shouldGoToCentralLossen(item);
    return false;
};
...
const hasNabewerkingFlag = (item) => {
    ...
    return Object.entries(item).some(([key, value]) => {
        ...
        if (!/(nabewerk|post.?process)/.test(keyLower))
            return false;
...
const LossenView = ({ stationId, appId, products = [] }) => {
        ...
        if (e.key === 'Enter') {
            ...
            if (code === QR_CODE_OK_CONFIRMATION && selectedProduct) {
                ...
                if (isAdvancedStation) {
                    ...
                    notify(t("lossen.measurement_required_for_ok_qr", "Let op: Voor Lossen is een meting verplicht. Vul de meetwaarden in op het scherm in plaats van de OK-QR te scannen."));
                }
                return;
            }
            const found = items.find(i => (i.lotNumber || "").toLowerCase() === code.toLowerCase() ||
    ...
    useEffect(() => {
        ...
        const processData = (sourceData) => {
            ...
            const filtered = sourceData.filter((item) => {
                ...
                const isDefRejected = inspectionStatus === "AFKEUR" ||
                ...
                if (isTempRejected && !isNabewerkingStation)
                    return false;
                ...
                if (!isDownstreamStation) {
                    const isLossenCandidate = stepUpper.includes("LOSSEN") ||
                ...
                if (currentStationNorm === "LOSSEN" || currentStationNorm === LOSSEN_1218_STATION_NORM) {
                    ...
                    const movedPastLossen = !isLossen1218Station &&
                        last === "LOSSEN" &&
                        current &&
                        current !== "LOSSEN" &&
                        current !== LOSSEN_1218_STATION_NORM;
                    const isDownstreamNow = !isLossen1218Station &&
                    ...
                    const isCentralHardBlocked = !isLossen1218Station &&
                        (isLossen1218Owned ||
                            movedPastLossen ||
                            isDownstreamNow ||
                    ...
                    if (user && user.allowedStations && user.allowedStations.length > 0) {
                        ...
                        const expandedUserTargets = Array.from(new Set(userTargets.flatMap((target) => {
            ...
            const fallbackFiltered = currentStationNorm === LOSSEN_1218_STATION_NORM && filtered.length === 0
                ? sourceData.filter((item) => {
                    ...
                    const isRejected = inspectionStatus === "AFKEUR" ||
                        statusUpper === "REJECTED" ||
                        statusUpper === "AFKEUR" ||
                        stepUpper === "REJECTED";
                    if (isRejected)
                    ...
                    if (isClosed)
                        return false;
                    const isDownstreamNow = currentNorm.includes("BM01") ||
                    ...
                    if (isDownstreamNow)
                        return false;
                    const alreadyLeftLossen = lastNorm.includes("LOSSEN") &&
                        currentNorm &&
                        currentNorm !== "LOSSEN" &&
                        currentNorm !== LOSSEN_1218_STATION_NORM;
                    if (alreadyLeftLossen)
                        return false;
                    ...
                    const lossenMatches = stepUpper.includes("LOSSEN") ||
                    ...
                    if (hasOriginInSet([originNorm], LOSSEN_1218_BH18_STATIONS) && sourceMatches && lossenMatches && shouldGoToCentralLossen(item))
                        return false;
            ...
            const finalItems = currentStationNorm === "LOSSEN"
                ? sorted.filter((item) => {
                    ...
                    if (isBh18For1218)
                        return false;
                    const isDownstream = current.includes("NABEWERK") ||
                        step.includes("NABEWERK") ||
                        status.includes("NABEWERK") ||
                        current.includes("BM01") ||
                        step.includes("INSPECTIE") ||
    ...
    const handleItemClick = (item) => {
        ...
        if (supportsSeriesGrouping && isFlensItem(item)) {
            if (item?.seriesGroupId) {
                sameSeries = items.filter((seriesItem) => seriesItem.seriesGroupId === item.seriesGroupId && isSeriesEligibleItem(seriesItem));
            }
    ...
    const viewTitle = useMemo(() => {
        if (isBM01 || isMazak || isNabewerking || isNaharding)
            return t('bm01.to_offer');
        if (currentStationNorm === "LOSSEN" || currentStationNorm === LOSSEN_1218_STATION_NORM)
            return t('lossen.wait_for_unload');
    ...
    const groupedSeries = useMemo(() => {
        if (!supportsSeriesGrouping)
            return new Map();
        const grouped = new Map();
        items.forEach((item) => {
            if (!isFlensItem(item))
                return; // Only allow grouping for FL items
    ...
    const handlePostProcessingFinish = async (status, data, productOverride = null) => {
        ...
        try {
            if (status === "completed" && isAdvancedStation) {
                ...
                await logActivity(user?.uid || "system", "POST_PROCESS_COMPLETE", `${stationId} afgerond${finishType === "archive" ? " en gearchiveerd" : (finishType === "post_inspection" ? " → Naharding" : " → BM01")}: lot ${product.lotNumber || productId}`);
                if (selectedProductRef.current?.id === product.id)
                    handleCloseModal();
                return;
            }
            if (status === "rejected") {
                ...
                await logActivity(user?.uid || "system", "QUALITY_REJECT_FINAL", `Lossen Definitieve afkeur en gearchiveerd: lot ${product.lotNumber || productId}`);
                if (selectedProductRef.current?.id === product.id)
                    handleCloseModal();
                return;
            }
            ...
            await logActivity(user?.uid || "system", status === "completed" ? "POST_PROCESS_COMPLETE" : "QUALITY_TEMP_REJECT", `Lossen afhandeling: lot ${product.lotNumber || productId}, station ${stationId}, status ${status}`);
            if (selectedProductRef.current?.id === product.id)
                handleCloseModal();
        }
        catch (error) {
    ...
    return (_jsxs("div", { className: "flex flex-col h-full overflow-hidden bg-white", children: [_jsx("style", { children: `
      ...
      ` }), showActionModal && selectedProduct && (_jsx("div", { className: "fixed z-[9999]", children: _jsx(ProductReleaseModal, { isOpen: true, product: selectedProduct, bulkProducts: bulkSeriesProducts, forceLossenMode: true, onClose: handleCloseModal, appId: appId, activeOperators: activeOperators, autoFocus: false }) })), _jsxs(_Fragment, { children: [_jsxs("div", { className: "flex-shrink-0 p-3 pb-2 space-y-2", children: [_jsxs("div", { className: "flex justify-between items-end", children: [_jsxs("div", { className: "flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg border border-blue-100 w-fit", children: [_jsx("div", { className: "w-2 h-2 bg-blue-500 rounded-full pulse-text" }), _jsxs("span", { className: "text-xs font-black text-blue-600 uppercase tracking-widest", children: ["\uD83D\uDD0D ", t('lossen.ready_to_scan', 'Klaar voor scan')] })] }), _jsxs("button", { onClick: () => setScannerMode(!scannerMode), className: `flex items-center gap-2 px-4 py-2 rounded-lg border-2 font-bold text-xs uppercase tracking-widest transition-all ${scannerMode ? 'bg-purple-100 border-purple-200 text-purple-700' : 'bg-white border-slate-200 text-slate-400'}`, title: scannerMode ? t("digitalplanning.terminal.scanner_keyboard_hidden", "Toetsenbord verborgen (Scanner modus)") : t("digitalplanning.terminal.normal_input", "Normale invoer"), children: [scannerMode ? _jsx(ScanBarcode, { size: 16 }) : _jsx(Keyboard, { size: 16 }), scannerMode ? t("digitalplanning.terminal.scanner_mode", "Scanner modus") : t("digitalplanning.terminal.keyboard", "Toetsenbord")] })] }), _jsxs("div", { className: "relative", children: [_jsx(ScanBarcode, { className: "absolute left-4 top-1/2 -translate-y-1/2 text-blue-500 transition-all scan-pulse", size: 24 }), _jsx("input", { ref: scanInputRef, type: "text", autoFocus: true, value: scanInput, onChange: (e) => setScanInput(e.target.value), inputMode: scannerMode ? "none" : "text", virtualKeyboardPolicy: scannerMode ? "manual" : "auto", onKeyDown: handleScan, placeholder: t("digitalplanning.terminal.scan_lot_or_order", "Scan lotnummer of order..."), className: "w-full pl-14 pr-4 py-4 bg-white border-2 border-blue-100 focus:border-blue-500 focus:ring-2 focus:ring-blue-300 rounded-2xl font-bold text-lg shadow-sm outline-none transition-all placeholder:text-slate-300" })] })] }), _jsx("div", { className: "flex-1 overflow-y-auto custom-scrollbar px-3 space-y-2", style: { paddingBottom: "max(8rem, env(safe-area-inset-bottom))" }, children: items.length === 0 ? (_jsxs("div", { className: "p-12 text-center bg-slate-50 rounded-[40px] border-2 border-dashed border-slate-200 opacity-40", children: [_jsx(Package, { size: 48, className: "mx-auto mb-4 text-slate-300" }), _jsx("p", { className: "text-[10px] font-black uppercase tracking-widest text-slate-400", children: t('lossen.no_incoming_items', { station: stationId }) })] })) : (_jsxs("div", { className: "flex flex-col gap-2", children: [_jsxs("div", { className: "flex items-center gap-2 mb-2 ml-2", children: [_jsx(ArrowRight, { size: 14, className: "text-emerald-500" }), _jsxs("h3", { className: "text-[10px] font-black text-slate-400 uppercase tracking-widest", children: [viewTitle, " (", items.length, ")"] })] }), displayRows.map((item) => {
                                    if (item.isSeriesHeader) {
                                        const isCollapsed = !!collapsedGroups[item.seriesGroupId];
                                        return (_jsx("div", { onClick: () => handleItemClick(item.seriesUnits[0]), className: "bg-blue-50 border border-blue-200 rounded-[14px] p-2.5 cursor-pointer w-full", children: _jsxs("div", { className: "flex items-center justify-between gap-2", children: [_jsxs("div", { children: [_jsx("p", { className: "text-[7px] font-black text-blue-500 uppercase tracking-widest", children: t("digitalplanning.terminal.series", "Serie") }), _jsxs("p", { className: "text-sm font-black text-blue-900", children: [t("productionStartModal.labels.order", "Order"), " ", item.orderId] }), _jsx("p", { className: "text-[8px] font-bold text-blue-700 uppercase", children: t("digitalplanning.terminal.series_count", "Serie {{count}} stuks", { count: item.seriesCount }) })] }), _jsx("button", { onClick: (e) => {
                                                            e.stopPropagation();
                                                            setCollapsedGroups((prev) => ({
                                                                ...prev,
                                                                [item.seriesGroupId]: !prev[item.seriesGroupId],
                                                            }));
                                                        }, className: "inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white border border-blue-200 text-blue-700 text-[7px] font-black uppercase flex-shrink-0", children: isCollapsed ? _jsx(ChevronRight, { size: 10 }) : _jsx(ChevronDown, { size: 10 }) })] }) }, item.id));
                                    }
                                    return (_jsxs("div", { onClick: () => handleItemClick(item), className: `bg-white border rounded-[14px] p-3 shadow-sm hover:border-emerald-300 hover:shadow-md transition-all group animate-in slide-in-from-bottom-2 cursor-pointer w-full
                      ${selectedProduct?.id === item.id ? 'border-purple-400 ring-2 ring-purple-200' : 'border-slate-100'}`, children: [_jsxs("div", { className: "flex justify-between items-start gap-3", children: [_jsxs("div", { className: "text-left flex-1 min-w-0", children: [_jsx("span", { className: "text-[7px] font-black text-slate-400 uppercase block mb-0.5", children: t('lossen.lot_number') }), _jsx("span", { className: "font-black text-slate-900 text-2xl tracking-tighter italic", children: item.lotNumber }), _jsx("p", { className: "text-[10px] font-bold text-slate-600 mt-0.5", children: item.item })] }), _jsx("div", { className: "px-1.5 py-0.5 bg-emerald-100 text-emerald-700 rounded-lg text-[7px] font-black uppercase whitespace-nowrap", children: t('lossen.received') })] }), _jsxs("div", { className: "bg-slate-50 rounded-lg p-1.5 border border-slate-100 mt-1.5", children: [_jsx("p", { className: "text-[7px] font-black text-slate-400 uppercase tracking-widest mb-0.5", children: t('lossen.manufactured_item') }), _jsx("p", { className: "text-[9px] font-mono font-bold text-slate-700 truncate", children: item.itemCode }), item.lastStation && (_jsxs("div", { className: "flex items-center gap-1 mt-1.5 pt-1.5 border-t border-slate-200/60 opacity-80", children: [_jsx(History, { size: 8, className: "text-blue-500" }), _jsxs("span", { className: "text-[6.5px] font-black text-slate-500 uppercase italic", children: [isBM01 ? t('lossen.from') + ": " : t('lossen.origin') + ": ", item.lastStation] })] }))] })] }, item.id));
                                })] })) })] })] }));
};
