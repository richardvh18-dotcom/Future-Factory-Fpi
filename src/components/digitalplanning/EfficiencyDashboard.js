...
import { Clock, AlertTriangle, TrendingUp, Activity, Search, Timer, BrainCircuit, History } from 'lucide-react';
...
import { formatMinutes, getEfficiencyColor } from '../../utils/efficiencyCalculator';
...
const EfficiencyDashboard = () => {
    ...
    const getTrackingDurationMinutes = (log) => {
        ...
        const start = toDateValue(log?.timestamps?.station_start ||
            log?.timestamps?.started ||
            log?.startTime ||
            log?.startedAt);
        if (start) {
            const end = toDateValue(log?.timestamps?.finished ||
    ...
    const matchesDepartmentFilter = (row) => {
        ...
        const candidates = [
            ...
            .map(normalizeText)
            .filter(Boolean);
        return candidates.some((candidate) => candidate === filter || candidate.includes(filter) || filter.includes(candidate));
    };
    const matchesMachineFilter = (row) => {
        if (machineFilter === 'ALL')
            return true;
        const filter = normalizeText(machineFilter);
        const machine = normalizeText(normalizeMachine(row.machine || row.originMachine || row.currentStation || row.lastStation));
        if (!machine)
            return false;
    ...
    const machineOptions = useMemo(() => {
        ...
        planningOrders.forEach((order) => {
            ...
            const filter = normalizeText(departmentFilter);
            if (departmentFilter !== 'ALL' && !(dept === filter || dept.includes(filter) || filter.includes(dept)))
                return;
            options.add(machine);
        });
    ...
    useEffect(() => {
        ...
        const unsubRootStandards = onSnapshot(collection(db, ...readPaths.EFFICIENCY_HOURS), (snapshot) => {
    ...
    const dashboardData = useMemo(() => {
        ...
        planningOrders.forEach((order) => {
            ...
            if (docId && !orderById.has(docId))
                orderById.set(docId, order);
        });
        ...
        let processed = standards.map(std => {
            ...
            const relatedLogs = tracking.filter(t => String(t.orderId || t.orderNumber) === stdOrderId);
            ...
            const eventDates = [
                ...
                (() => {
                    const delivery = resolveDeliveryDate(planningOrder?.deliveryDate, planningOrder?.plannedDeliveryDate, planningOrder?.dueDate, planningOrder?.deadline);
                    return delivery;
                })(),
                (() => {
                    const delivery = resolveDeliveryDate(planningOrder?.deliveryDate, planningOrder?.plannedDeliveryDate, planningOrder?.dueDate, planningOrder?.deadline);
            ...
            const inSelectedPeriod = viewMode === 'archive'
                ? true
        ...
        const knownOrderIds = new Set(standards
        ...
        if (searchTerm) {
            ...
            processed = processed.filter(i => String(i.orderId).toLowerCase().includes(lower));
        }
        // Sorteren: Orders met meeste aandacht nodig bovenaan (laagste efficiency eerst, maar wel met activiteit)
        processed.sort((a, b) => {
            if (a.actualMinutes === 0 && b.actualMinutes > 0)
                return 1;
            if (a.actualMinutes > 0 && b.actualMinutes === 0)
                return -1;
    ...
    if (showAiAnalysis) {
        return _jsx(AiPredictionView, { onClose: () => setShowAiAnalysis(false) });
    }
    return (_jsxs("div", { className: "space-y-6 p-6 max-w-7xl mx-auto", children: [_jsxs("div", { className: "grid grid-cols-1 md:grid-cols-4 gap-4", children: [_jsxs("div", { className: "bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4", children: [_jsx("div", { className: `p-3 rounded-xl ${dashboardData.kpi.avgEfficiency >= 85 ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`, children: _jsx(Activity, { size: 24 }) }), _jsxs("div", { children: [_jsx("div", { className: "text-sm text-slate-500 font-bold uppercase tracking-wider", children: t('efficiency_dashboard.avg_efficiency') }), _jsxs("div", { className: "text-2xl font-black text-slate-800", children: [dashboardData.kpi.avgEfficiency, "%"] })] })] }), _jsxs("div", { className: "bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4", children: [_jsx("div", { className: "p-3 rounded-xl bg-blue-100 text-blue-600", children: _jsx(TrendingUp, { size: 24 }) }), _jsxs("div", { children: [_jsx("div", { className: "text-sm text-slate-500 font-bold uppercase tracking-wider", children: t('efficiency_dashboard.active_orders') }), _jsx("div", { className: "text-2xl font-black text-slate-800", children: dashboardData.kpi.activeCount })] })] }), _jsxs("div", { className: "bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4", children: [_jsx("div", { className: "p-3 rounded-xl bg-purple-100 text-purple-600", children: _jsx(Clock, { size: 24 }) }), _jsxs("div", { children: [_jsx("div", { className: "text-sm text-slate-500 font-bold uppercase tracking-wider", children: t('efficiency_dashboard.hours_spent') }), _jsx("div", { className: "text-2xl font-black text-slate-800", children: formatMinutes(dashboardData.kpi.totalActual) })] })] }), _jsxs("div", { className: "bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4", children: [_jsx("div", { className: `p-3 rounded-xl ${dashboardData.kpi.overrunCount > 0 ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-400'}`, children: _jsx(AlertTriangle, { size: 24 }) }), _jsxs("div", { children: [_jsx("div", { className: "text-sm text-slate-500 font-bold uppercase tracking-wider", children: t('efficiency_dashboard.overruns') }), _jsx("div", { className: "text-2xl font-black text-slate-800", children: dashboardData.kpi.overrunCount })] })] })] }), _jsxs("div", { className: "flex flex-col md:flex-row gap-4 justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm", children: [_jsxs("select", { value: departmentFilter, onChange: (e) => setDepartmentFilter(e.target.value), className: "bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-2.5 font-bold outline-none", children: [_jsx("option", { value: "ALL", children: "Alle Afdelingen" }), _jsx("option", { value: "FITTINGS", children: "Fittings" }), _jsx("option", { value: "PIPES", children: "Pipes" }), _jsx("option", { value: "SPOOLS", children: "Spools" })] }), _jsxs("select", { value: machineFilter, onChange: (e) => setMachineFilter(e.target.value), className: "bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-2.5 font-bold outline-none", children: [_jsx("option", { value: "ALL", children: "Alle Machines" }), machineOptions.map((machine) => (_jsx("option", { value: machine, children: machine }, machine)))] }), _jsxs("div", { className: "flex items-center gap-2 w-full md:w-auto", children: [_jsx(Search, { className: "text-slate-400", size: 20 }), _jsx("input", { type: "text", placeholder: t('efficiency_dashboard.search_placeholder'), className: "bg-transparent border-none focus:ring-0 text-slate-700 font-medium w-full", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value) })] }), _jsxs("div", { className: "flex flex-wrap gap-2 items-center", children: [_jsxs("button", { onClick: () => setShowAiAnalysis(true), className: "flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-xl shadow-md font-black text-[10px] uppercase tracking-wider hover:bg-purple-700 transition-all", children: [_jsx(BrainCircuit, { size: 16 }), _jsx("span", { className: "hidden sm:inline", children: "AI Analyse" })] }), _jsxs("div", { className: "flex items-center bg-slate-100 rounded-lg p-1 mr-2", children: [_jsx("button", { onClick: () => setViewMode('active'), className: `px-3 py-1.5 rounded-md text-xs font-bold transition-all ${viewMode === 'active' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`, children: t('efficiency_dashboard.view_active') }), _jsxs("button", { onClick: () => setViewMode('archive'), className: `flex items-center gap-1 px-3 py-1.5 rounded-md text-xs font-bold transition-all ${viewMode === 'archive' ? 'bg-white text-purple-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`, children: [_jsx(History, { size: 12 }), t('efficiency_dashboard.view_archive')] })] }), viewMode === 'archive' && (_jsx("select", { value: selectedYear, onChange: (e) => setSelectedYear(parseInt(e.target.value)), className: "bg-white border border-slate-200 text-slate-700 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block p-2 font-bold", children: [0, 1, 2, 3].map(offset => {
                                    const y = new Date().getFullYear() - offset;
                                    return _jsx("option", { value: y, children: y }, y);
                                }) })), viewMode === 'active' && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex bg-slate-100 rounded-lg p-1", children: [_jsx("button", { onClick: () => setFilterStatus('active'), className: `px-3 py-1.5 rounded-md text-xs font-bold transition-all ${filterStatus === 'active' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500'}`, children: t('efficiency_dashboard.filter_open') }), _jsx("button", { onClick: () => setFilterStatus('all'), className: `px-3 py-1.5 rounded-md text-xs font-bold transition-all ${filterStatus === 'all' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500'}`, children: t('efficiency_dashboard.filter_all') })] }), _jsxs("div", { className: "flex items-center gap-1 bg-slate-100 rounded-lg p-1", children: [_jsx("button", { onClick: () => setPeriodMode('day'), className: `px-2.5 py-1.5 rounded-md text-xs font-bold ${periodMode === 'day' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500'}`, children: "Dag" }), _jsx("button", { onClick: () => setPeriodMode('week'), className: `px-2.5 py-1.5 rounded-md text-xs font-bold ${periodMode === 'week' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500'}`, children: "Week" }), _jsx("button", { onClick: () => setPeriodMode('month'), className: `px-2.5 py-1.5 rounded-md text-xs font-bold ${periodMode === 'month' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500'}`, children: "Maand" })] }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx("button", { onClick: navigatePrevious, className: "px-2 py-1.5 rounded-md bg-slate-100 text-slate-700 text-xs font-bold", children: "Vorige" }), _jsx("span", { className: "text-xs font-bold text-slate-600 min-w-[150px] text-center", children: periodMode === 'day'
                                                    ? formatDate(selectedDate, 'dd-MM-yyyy')
                                                    : periodMode === 'month'
                                                        ? formatDate(selectedDate, 'MMMM yyyy')
                                                        : `Week ${getISOWeek(selectedDate)} - ${formatDate(selectedDate, 'yyyy')}` }), _jsx("button", { onClick: navigateNext, className: "px-2 py-1.5 rounded-md bg-slate-100 text-slate-700 text-xs font-bold", children: "Volgende" }), _jsx("button", { onClick: jumpToToday, className: "px-2 py-1.5 rounded-md bg-blue-500 text-white text-xs font-bold", children: "Vandaag" })] })] }))] })] }), _jsxs("div", { className: "grid grid-cols-1 gap-4", children: [dashboardData.items.map((item) => (_jsx("div", { className: "bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow", children: _jsxs("div", { className: "flex flex-col md:flex-row justify-between gap-6", children: [_jsxs("div", { className: "flex-1", children: [_jsxs("div", { className: "flex items-center gap-3 mb-1", children: [_jsx("span", { className: "text-lg font-black text-slate-800", children: item.orderId }), _jsx("span", { className: `px-2 py-1 rounded-md text-xs font-bold uppercase ${item.isOverrun ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'}`, children: item.isOverrun ? t('efficiency_dashboard.status_overrun') : t('efficiency_dashboard.status_on_schedule') })] }), _jsxs("div", { className: "text-sm font-bold text-blue-600 mb-2", children: [item.itemCode, " ", _jsx("span", { className: "text-slate-400", children: "\u2022" }), " ", item.item] }), _jsxs("div", { className: "text-xs text-slate-500 flex flex-wrap gap-4 items-center", children: [_jsxs("span", { children: [t('efficiency_dashboard.qty'), ": ", _jsx("b", { children: item.quantity })] }), _jsxs("span", { children: [t('efficiency_dashboard.produced'), ": ", _jsx("b", { children: item.producedQty })] }), _jsxs("span", { children: [t('efficiency_dashboard.norm'), ": ", _jsxs("b", { children: [Math.round(item.minutesPerUnit * 10) / 10, "m"] }), " / ", t('efficiency_dashboard.per_piece')] }), _jsxs("div", { className: "flex flex-wrap gap-2 mt-1", children: [item.productionTimeTotal > 0 && (_jsxs("span", { className: "px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs font-bold border border-blue-100", title: t('efficiency_dashboard.prod_tooltip'), children: [t('efficiency_dashboard.prod'), ": ", formatMinutes(item.productionTimeTotal)] })), item.postProcessingTimeTotal > 0 && (_jsxs("span", { className: "px-2 py-0.5 bg-purple-50 text-purple-700 rounded text-xs font-bold border border-purple-100", title: t('efficiency_dashboard.post_tooltip'), children: [t('efficiency_dashboard.post'), ": ", formatMinutes(item.postProcessingTimeTotal)] })), item.qcTimeTotal > 0 && (_jsxs("span", { className: "px-2 py-0.5 bg-amber-50 text-amber-700 rounded text-xs font-bold border border-amber-100", title: t('efficiency_dashboard.qc_excluded'), children: [t('efficiency_dashboard.qc'), ": ", formatMinutes(item.qcTimeTotal)] }))] })] })] }), _jsxs("div", { className: "flex-1 flex flex-col justify-center", children: [_jsxs("div", { className: "flex justify-between text-sm font-bold mb-2", children: [_jsxs("span", { className: "text-slate-600", children: [formatMinutes(item.actualMinutes), " ", t('efficiency_dashboard.spent')] }), _jsxs("div", { className: "text-right", children: [_jsx("span", { className: "text-slate-400 block text-[10px] uppercase", children: "Target" }), _jsx("span", { className: "text-slate-600", children: formatMinutes(item.standardTimeTotal) })] })] }), _jsx("div", { className: "h-4 bg-slate-100 rounded-full overflow-hidden", children: _jsx("div", { className: `h-full rounded-full transition-all duration-500 ${getEfficiencyColor(item.efficiency).replace('text-', 'bg-').split(' ')[1]}`, style: { width: `${Math.min(100, (item.actualMinutes / item.standardTimeTotal) * 100)}%` } }) })] }), _jsxs("div", { className: "flex flex-col items-end justify-center min-w-[100px]", children: [_jsxs("div", { className: `text-3xl font-black ${getEfficiencyColor(item.efficiency).split(' ')[0]}`, children: [Math.round(item.efficiency), "%"] }), _jsx("div", { className: "text-xs text-slate-400 font-bold uppercase", children: t('efficiency_dashboard.efficiency') }), _jsxs("div", { className: "mt-2 flex items-center gap-1 bg-purple-50 px-2 py-1 rounded border border-purple-100", title: `Gebaseerd op ${Math.round(item.aiConfidence / 10)} eerdere producties`, children: [_jsx(BrainCircuit, { size: 10, className: "text-purple-500" }), _jsxs("span", { className: "text-[9px] font-bold text-purple-700", children: ["AI: ~", formatMinutes(item.aiPredictedTotal)] })] })] })] }) }, item.orderId))), dashboardData.items.length === 0 && (_jsxs("div", { className: "text-center py-12 text-slate-400", children: [_jsx(Timer, { size: 48, className: "mx-auto mb-4 opacity-20" }), _jsx("p", { children: viewMode === 'active'
