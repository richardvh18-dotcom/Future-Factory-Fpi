 ...
 */
const TeamleaderPipesHub = React.memo((props) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
            ...
            if (docSnap.exists()) {
                ...
                const myDept = (data.departments || []).find((d) => d.slug === "pipes" || d.id === "pipes");
                if (myDept) {
    ...
    if (loading)
        return (_jsx("div", { className: "h-full flex items-center justify-center", children: _jsx(Loader2, { className: "animate-spin" }) }));
    const machineIds = stations.map((s) => s.name);
    return (_jsx(TeamleaderHub, { ...props, fixedScope: "pipe", departmentName: t('teamleader.pipe_productions', 'Pijp Productie'), allowedMachines: machineIds }));
});
