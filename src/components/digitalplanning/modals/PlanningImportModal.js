...
import { X, Upload, Loader2, Database, ShieldCheck, } from "lucide-react";
...
const PlanningImportModal = ({ isOpen, onClose, onSuccess, currentDepartment = "all" }) => {
    ...
    const SMART_SYNC_EXCLUDED_ORDER_IDS = useMemo(() => new Set([
        "N20024490",
        "N20024491",
        "N20024566",
        "N20024604",
            ...
            try {
                ...
                try {
                    ...
                    trackedSnap.docs.forEach((docEntry) => {
                        const path = String(docEntry?.ref?.path || "");
                        if (!path.includes("/tracked_products/"))
                            return;
                        const data = docEntry.data() || {};
    ...
    const isSmartSyncExcludedOrder = (order) => {
        ...
        if (SMART_SYNC_EXCLUDED_ORDER_IDS.has(orderId))
            return true;
        // 2. Check dynamic exclusion field from Firestore
        const existing = getExistingOrder(order);
        if (existing?.smartSyncExcluded === true)
            return true;
        if (existing?.smartSyncIncluded === false)
            return true;
        return false;
    };
    ...
    const getComparableReadyQty = (order) => {
        const produced = order?.inspectionApprovedQty ??
            order?.produced ??
            0;
    ...
    const isStatusAllowed = (status) => {
        ...
        if (s.includes("production completed") || s.includes("completed"))
            return false;
    ...
    const classifyByWc = (wc) => {
        ...
        if (QC_STATIONS.some(s => upper.includes(s)))
            return "qc";
    ...
    const processRawLNDump = (rawRows) => {
        const headerIdx = rawRows.findIndex((r) => r.some((c) => {
            const h = normalizeHeader(c);
            return (h === "production order" ||
                h === "productieorder" ||
                h === "ordernummer" ||
                h === "order number");
        }));
        ...
        const dataRows = rawRows.slice(headerIdx + 1);
        const findCol = (names) => headers.findIndex((h) => names.some((n) => h.includes(normalizeHeader(n))));
        const idx = {
        ...
        dataRows.forEach(row => {
            ...
            if (refOp) {
                ...
                if (!order._opRows)
                    order._opRows = {};
                if (!order._opRows[refOp])
                    order._opRows[refOp] = { derived: [], original: [] };
        ...
        addLog(t("digitalplanning.planning_import.logs.orders_consolidated", {
            count: result.length,
    ...
    const handleSheetChange = (sheetName, workbookOverride = null) => {
        ...
        try {
            ...
            if (!sheet) {
                addLog(t("digitalplanning.planning_import.logs.sheet_not_found", {
                    name: sheetName,
    ...
    const handleFile = async (e) => {
        ...
        try {
            ...
            addLog(t("digitalplanning.planning_import.logs.file_unreadable", "Bestand onleesbaar."), "error");
        }
        finally {
            setLoading(false);
        }
    ...
    const handlePasteImport = async () => {
        ...
        try {
            ...
            if (rows.length <= 2 && (rows[0]?.length || 0) > 40) {
                ...
                const headerStart = lowered.findIndex((c, i) => c === "datum" &&
                    lowered[i + 1] === "week" &&
                    lowered[i + 2] === "order");
                if (headerStart !== -1) {
                    ...
                    for (let i = 0; i < dataCells.length; i += headerLen) {
                        ...
                        if (!chunk.length)
                            continue;
                        while (chunk.length < headerLen)
                            chunk.push("");
                        if (chunk.some((v) => String(v || "").trim() !== ""))
                            rebuilt.push(chunk);
                    }
                    if (rebuilt.length > 1)
                        rows = rebuilt;
            ...
            setToDoOverrides({});
            addLog(t("digitalplanning.planning_import.logs.paste_rows_loaded", {
    ...
    const getDepartmentGroupMachines = (groupName) => {
        if (groupName === "fittings")
            return availableMachines.filter((machine) => isFittingsMachine(machine));
        if (groupName === "pipes")
            return availableMachines.filter((machine) => isPipesMachine(machine));
        if (groupName === "spools")
            return availableMachines.filter((machine) => isSpoolsMachine(machine));
        return availableMachines;
    };
    const toggleMachineSelection = (machineCode) => {
        setSelectedMachines((prev) => {
            if (prev.includes(machineCode))
                return prev.filter((m) => m !== machineCode);
    ...
    const normalizePoText = (value) => clean(value)
        .replace(/\s+/g, " ")
        .trim();
    const orderChangeMeta = useMemo(() => {
        ...
        effectiveValidOrders.forEach((order) => {
            ...
            const hasManualPlanOverride = Number.isFinite(existingPlanRaw) &&
            ...
            const hoursChanged = newPlannedHours !== null &&
    ...
    const displayData = useMemo(() => {
        ...
        if (readySyncFilter !== "all") {
            rows = rows.filter((d) => {
                ...
                if (readySyncFilter === "match")
                    return delta === 0;
                if (readySyncFilter === "higher")
                    return delta > 0;
                if (readySyncFilter === "lower")
                    return delta < 0;
                if (readySyncFilter === "mismatch")
                    return delta !== 0;
                return true;
            });
        }
        ...
        if (pasteMode) {
            ...
            if (!hoursOnlyMode) {
                rows = rows.filter((d) => {
                    ...
                    if (clean(d?.orderId) === "N20024607")
                        return false;
                    if (isSmartSyncExcludedOrder(d))
                        return false;
                    const meta = orderChangeMeta.get(d.id);
                ...
                rows.sort((a, b) => {
                    ...
                    if (Number(bEligible) !== Number(aEligible))
    ...
    const importCandidates = useMemo(() => {
        ...
        if (pasteMode) {
            ...
            rows = effectiveValidOrders.filter((d) => importMode === "overwrite" ||
                !isExistingOrder(d));
        }
        rows = rows.filter((d) => isAllowedBySelectedMachines(d));
        if (isFittingsScoped) {
    ...
    useEffect(() => {
        ...
        setSelectedOrderIds((prev) => {
            ...
            if (prev.size > 0 && importMode !== "overwrite")
                return prev;
            let candidates;
            if (pasteMode) {
    ...
    const importableCount = useMemo(() => importCandidates.length, [importCandidates]);
    const deliveryBuckets = useMemo(() => {
        return displayData.reduce((acc, order) => {
    ...
    const startImport = async () => {
        ...
        try {
            ...
            for (let i = 0; i < toImport.length; i += CHUNK) {
                ...
                setImportProgressLabel(t("digitalplanning.planning_import.progress_chunks", {
                    current: chunkNumber,
                    total: totalChunks,
                ...
                setImportEtaLabel(remainingChunks > 0
    ...
    if (!isOpen)
        return null;
    return (_jsx("div", { className: "fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/95 backdrop-blur-md", children: _jsxs("div", { className: "bg-white w-full max-w-[96vw] h-[96vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden border border-white/20 text-left relative", children: [importing && (_jsx("div", { className: "absolute inset-0 z-[120] flex items-center justify-center bg-slate-900/60 backdrop-blur-md rounded-[3rem]", children: _jsxs("div", { className: "bg-white border border-slate-200 rounded-[2.5rem] p-10 shadow-2xl w-full max-w-2xl flex flex-col gap-8", children: [_jsxs("div", { className: "flex items-center gap-4 text-blue-600", children: [_jsx(Loader2, { className: "animate-spin", size: 40 }), _jsx("h3", { className: "text-3xl font-black uppercase tracking-tight", children: t("digitalplanning.planning_import.progress_busy", "Importeren...") })] }), _jsxs("div", { className: "flex justify-between items-end text-base font-black uppercase tracking-widest text-slate-500", children: [_jsx("span", { className: "text-slate-700", children: importProgressLabel }), _jsxs("span", { className: "flex items-center gap-4", children: [importEtaLabel ? _jsx("span", { className: "text-slate-400 bg-slate-100 px-4 py-1.5 rounded-xl", children: importEtaLabel }) : null, _jsxs("span", { className: "text-blue-600 text-3xl leading-none", children: [importProgressPct, "%"] })] })] }), _jsx("div", { className: "h-8 w-full bg-slate-100 rounded-full overflow-hidden border-2 border-slate-200/60 shadow-inner", children: _jsx("div", { className: "h-full bg-gradient-to-r from-blue-500 to-blue-400 transition-all duration-300 ease-out flex items-center justify-end pr-3 shadow-lg", style: { width: `${importProgressPct}%` }, children: importProgressPct > 5 && _jsx("div", { className: "w-3 h-3 rounded-full bg-white/50 animate-pulse" }) }) })] }) })), _jsxs("div", { className: "p-5 border-b flex justify-between items-center bg-slate-50", children: [_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: "bg-blue-600 p-3 rounded-[1.1rem] text-white shadow-xl", children: _jsx(Database, { size: 22 }) }), _jsxs("div", { children: [_jsx("h2", { className: "text-xl font-black text-slate-800 uppercase tracking-tight italic leading-none", children: t("digitalplanning.planning_import.title", "Planning Import") }), _jsx("p", { className: "text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1", children: "v4.7 \u2022 Extended Dossier Support" })] })] }), _jsx("button", { onClick: onClose, className: "p-2 hover:bg-slate-200 rounded-full text-slate-400 transition-all", children: _jsx(X, { size: 24 }) })] }), _jsx("div", { className: "flex-1 overflow-hidden flex", children: _jsx("div", { className: "flex-1 p-8 overflow-hidden bg-white custom-scrollbar", children: fileData.length === 0 ? (_jsx("div", { className: "h-full rounded-[4rem] border-2 border-slate-100 p-8 flex flex-col gap-6 bg-slate-50/40", children: _jsxs("div", { onClick: () => fileInputRef.current?.click(), onDragOver: handleDragOver, onDragLeave: handleDragLeave, onDrop: handleDrop, className: `flex-1 border-4 border-dashed rounded-[3rem] flex flex-col items-center justify-center transition-all cursor-pointer group text-center min-h-[320px] ${isDragging
                                    ? "border-blue-500 bg-blue-50/80 scale-[1.02]"
                                    : "border-slate-200 hover:border-blue-400 hover:bg-blue-50/50 bg-white"}`, children: [_jsx("div", { className: `w-24 h-24 rounded-full flex items-center justify-center mb-6 transition-transform ${isDragging ? "bg-blue-600 text-white scale-110 shadow-xl" : "bg-blue-100 text-blue-600 group-hover:scale-110"}`, children: loading ? _jsx(Loader2, { className: "animate-spin", size: 50 }) : _jsx(Upload, { size: 50 }) }), _jsx("h3", { className: "text-2xl font-black text-slate-700 uppercase", children: isDragging ? t("digitalplanning.planning_import.drop_to_import", "Laat los om te importeren") : t("digitalplanning.planning_import.select_ln_export", "Selecteer LN Export") }), _jsx("p", { className: "text-slate-400 mt-2 font-medium italic", children: t("digitalplanning.planning_import.drag_drop_support", "Sleep een bestand hierheen of klik om te bladeren") }), _jsx("input", { type: "file", ref: fileInputRef, onChange: handleFile, accept: ".xlsx,.xlsm", className: "hidden" })] }) })) : (_jsxs("div", { className: "h-full min-h-0 flex flex-col gap-8", children: [_jsxs("div", { className: "bg-slate-900 p-4 rounded-[2.5rem] flex justify-between items-start shadow-2xl gap-4", children: [_jsxs("div", { className: "flex-1 min-w-0 text-white", children: [_jsxs("div", { className: "flex flex-wrap items-end gap-4", children: [_jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: "text-[10px] font-black text-blue-400 uppercase ml-1 mb-1 tracking-widest", children: t("digitalplanning.planning_import.department_group_label", "Afdelingsgroep") }), _jsxs("select", { value: machineGroupFilter, onChange: (e) => setMachineGroupFilter(e.target.value), disabled: isFittingsScoped, className: "bg-white/10 border border-white/20 rounded-xl px-3 py-2 font-bold text-xs text-white outline-none focus:border-blue-500 disabled:opacity-60 disabled:cursor-not-allowed", children: [_jsx("option", { value: "all", className: "text-slate-800", children: t("digitalplanning.planning_import.machine_group_all", "ALLES") }), _jsx("option", { value: "fittings", className: "text-slate-800", children: t("digitalplanning.planning_import.machine_group_fittings", "FITTINGS") }), _jsx("option", { value: "pipes", className: "text-slate-800", children: t("digitalplanning.planning_import.machine_group_pipes", "PIPES") }), _jsx("option", { value: "spools", className: "text-slate-800", children: t("digitalplanning.planning_import.machine_group_spools", "SPOOLS") })] })] }), _jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: "text-[10px] font-black text-blue-400 uppercase ml-1 mb-1 tracking-widest", children: t("digitalplanning.planning_import.status_label", "Status") }), _jsxs("select", { value: statusFilter, onChange: (e) => setStatusFilter(e.target.value), className: "bg-white/10 border border-white/20 rounded-xl px-3 py-2 font-bold text-xs text-white outline-none focus:border-blue-500", children: [_jsx("option", { value: "all", className: "text-slate-800", children: t("digitalplanning.planning_import.status_all", "ALLES") }), _jsx("option", { value: "new", className: "text-slate-800", children: t("digitalplanning.planning_import.status_new", "NIEUW") }), _jsx("option", { value: "existing", className: "text-slate-800", children: t("digitalplanning.planning_import.status_existing", "BESTAAND") })] })] }), _jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: "text-[10px] font-black text-blue-400 uppercase ml-1 mb-1 tracking-widest", children: t("digitalplanning.planning_import.ready_sync_label", "Wikkel LN vs FF") }), _jsxs("select", { value: readySyncFilter, onChange: (e) => setReadySyncFilter(e.target.value), className: "bg-white/10 border border-white/20 rounded-xl px-3 py-2 font-bold text-xs text-white outline-none focus:border-blue-500", children: [_jsx("option", { value: "all", className: "text-slate-800", children: t("digitalplanning.planning_import.ready_sync_all", "ALLES") }), _jsx("option", { value: "match", className: "text-slate-800", children: t("digitalplanning.planning_import.ready_sync_match", "GELIJK") }), _jsx("option", { value: "higher", className: "text-slate-800", children: t("digitalplanning.planning_import.ready_sync_higher", "LN > FF") }), _jsx("option", { value: "lower", className: "text-slate-800", children: t("digitalplanning.planning_import.ready_sync_lower", "LN < FF") }), _jsx("option", { value: "mismatch", className: "text-slate-800", children: t("digitalplanning.planning_import.ready_sync_mismatch", "VERSCHIL") })] })] }), _jsxs("div", { className: "flex flex-wrap gap-2", children: [_jsx("button", { onClick: () => selectMachines(availableMachines), className: "px-2.5 py-1.5 bg-white/10 border border-white/20 text-white rounded-lg font-black uppercase text-[10px] tracking-widest hover:bg-white/20", children: t("digitalplanning.planning_import.select_all", "Alles selecteren") }), _jsx("button", { onClick: () => selectMachines([]), className: "px-2.5 py-1.5 bg-white/10 border border-white/20 text-white rounded-lg font-black uppercase text-[10px] tracking-widest hover:bg-white/20", children: t("digitalplanning.planning_import.clear_selection", "Leegmaken") }), _jsx("button", { onClick: () => setVisibleSelection(true), className: "px-2.5 py-1.5 bg-white/10 border border-white/20 text-white rounded-lg font-black uppercase text-[10px] tracking-widest hover:bg-white/20", children: t("digitalplanning.planning_import.all_visible", "Alles zichtbaar") }), _jsx("button", { onClick: () => setVisibleSelection(false), className: "px-2.5 py-1.5 bg-white/10 border border-white/20 text-white rounded-lg font-black uppercase text-[10px] tracking-widest hover:bg-white/20", children: t("digitalplanning.planning_import.all_hidden", "Alles verborgen") }), _jsx("div", { className: "border-l border-white/30 pl-2" }), _jsxs("label", { className: "flex items-center gap-2 px-2.5 py-1.5 bg-yellow-500/20 border border-yellow-500/40 rounded-lg cursor-pointer hover:bg-yellow-500/30 transition-all", children: [_jsx("input", { type: "checkbox", checked: hoursOnlyMode, onChange: (e) => setHoursOnlyMode(e.target.checked), className: "w-3.5 h-3.5 rounded accent-yellow-500" }), _jsxs("span", { className: "text-yellow-200 font-black uppercase text-[10px] tracking-widest", children: ["\uD83D\uDCCB ", t("digitalplanning.planning_import.hours_only_mode", "Alleen Uren")] })] })] })] }), _jsx("div", { className: "mt-3 flex flex-wrap gap-2 max-h-16 overflow-y-auto pr-1", children: availableMachines.map((machineCode) => {
                                                        const selected = selectedMachines.includes(machineCode);
                                                        return (_jsx("button", { onClick: () => toggleMachineSelection(machineCode), className: `px-2.5 py-1 rounded-lg font-black uppercase text-[10px] tracking-widest border transition-all ${selected ? "bg-blue-500 text-white border-blue-500" : "bg-white/5 text-slate-200 border-white/20 hover:bg-white/15"}`, children: machineCode }, machineCode));
                                                    }) })] }), _jsxs("div", { className: "text-right text-white shrink-0", children: [_jsx("p", { className: "text-[10px] font-black opacity-40 uppercase tracking-widest", children: t("digitalplanning.planning_import.found_orders", "Gevonden Orders") }), _jsx("p", { className: "text-3xl font-black tracking-tighter", children: displayData.length }), _jsx("p", { className: "text-[10px] mt-1 text-emerald-200 font-black uppercase tracking-widest", children: t("digitalplanning.planning_import.selected_machines_count", { count: selectedMachines.length, defaultValue: "Machines in import: {{count}}" }) }), _jsx("p", { className: "text-[10px] mt-1 text-blue-200 font-black uppercase tracking-widest", children: t("digitalplanning.planning_import.in_planning", { count: displayData.filter((order) => selectedOrderIds.has(order.id)).length, defaultValue: "In Planning: {{count}}" }) }), _jsxs("div", { className: "mt-2 flex flex-wrap justify-end gap-2", children: [_jsx("span", { className: "px-2 py-1 rounded-lg bg-red-500/20 text-red-200 text-[10px] font-black uppercase tracking-widest", children: t("digitalplanning.planning_import.bucket_overdue", { count: deliveryBuckets.overdue, defaultValue: "Achter: {{count}}" }) }), _jsx("span", { className: "px-2 py-1 rounded-lg bg-amber-500/20 text-amber-200 text-[10px] font-black uppercase tracking-widest", children: t("digitalplanning.planning_import.bucket_current", { count: deliveryBuckets.current, defaultValue: "Deze week: {{count}}" }) }), _jsx("span", { className: "px-2 py-1 rounded-lg bg-emerald-500/20 text-emerald-200 text-[10px] font-black uppercase tracking-widest", children: t("digitalplanning.planning_import.bucket_upcoming", { count: deliveryBuckets.upcoming, defaultValue: "Komend: {{count}}" }) })] })] })] }), _jsx("div", { className: "border border-slate-100 rounded-[2.5rem] overflow-hidden bg-white shadow-sm flex-1 min-h-0 overflow-y-auto", children: _jsxs("table", { className: "w-full text-left text-[11px] border-collapse", children: [_jsx("thead", { className: "sticky top-0 z-10 bg-slate-50 text-slate-400 font-black uppercase tracking-wider border-b", children: _jsxs("tr", { children: [_jsx("th", { className: "px-4 py-3 sticky top-0 bg-slate-50", children: t("digitalplanning.planning_import.table_order", "Order") }), _jsx("th", { className: "px-3 py-3 sticky top-0 bg-slate-50", children: t("digitalplanning.planning_import.table_machine", "Machine") }), _jsx("th", { className: "px-3 py-3 sticky top-0 bg-slate-50", children: t("digitalplanning.planning_import.table_product", "Product") }), _jsx("th", { className: "px-3 py-3 sticky top-0 bg-slate-50 text-center", children: t("digitalplanning.planning_import.table_delivery_date", "Leverdatum") }), _jsx("th", { className: "px-2 py-3 sticky top-0 bg-slate-50 text-center", children: t("digitalplanning.planning_import.table_status", "Status") }), _jsx("th", { className: "px-2 py-3 sticky top-0 bg-slate-50 text-center", children: t("digitalplanning.planning_import.table_quantity", "Orderhoeveelheid") }), _jsx("th", { className: "px-2 py-3 sticky top-0 bg-slate-50 text-center", children: t("digitalplanning.planning_import.table_todo_qty", "Te maken") }), _jsx("th", { className: "px-2 py-3 sticky top-0 bg-slate-50 text-center", children: t("digitalplanning.planning_import.table_ready_qty", "Hoeveelheid gereed (LN/FF)") }), _jsx("th", { className: "px-2 py-3 sticky top-0 bg-slate-50 text-center w-[100px]", children: t("digitalplanning.planning_import.table_extra_code", "ExtraCode") }), _jsx("th", { className: "px-2 py-3 sticky top-0 bg-slate-50", children: t("digitalplanning.planning_import.table_po_text", "PO Text") }), _jsx("th", { className: "px-3 py-3 sticky top-0 bg-slate-50 text-center", children: t("digitalplanning.planning_import.table_plan_hours", "Plan Uren") }), _jsx("th", { className: "px-3 py-3 sticky top-0 bg-slate-50 text-right pr-4", children: _jsxs("div", { className: "flex items-center justify-end gap-2", children: [_jsx("span", { className: "text-[10px]", children: t("digitalplanning.planning_import.table_in_planning", "In Planning") }), _jsx("input", { type: "checkbox", className: "w-4 h-4 rounded accent-blue-600 cursor-pointer border-slate-300", checked: displayData.length > 0 && displayData.every(order => selectedOrderIds.has(order.id)), onChange: (e) => setVisibleSelection(e.target.checked), title: t("digitalplanning.planning_import.toggle_all_visible", "Selecteer alle zichtbare orders") })] }) })] }) }), _jsx("tbody", { className: "divide-y divide-slate-50", children: displayData.slice(0, 50).map((order) => {
                                                    ...
                                                    const isSmartUnchangedExisting = importMode === "smart_update" &&
                                                        !hoursOnlyMode &&
                                                        isExisting &&
                                                    ...
                                                    return (_jsxs("tr", { className: `hover:bg-blue-50/30 transition-all ${!order.isValidForImport ? 'opacity-30 grayscale italic' : ''}`, children: [_jsx("td", { className: "px-4 py-1.5 font-black text-slate-900 whitespace-nowrap leading-tight", children: order.orderId }), _jsx("td", { className: "px-3 py-1.5", children: _jsx("span", { className: "bg-blue-100 text-blue-700 px-2 py-[2px] rounded-lg font-black text-[10px] uppercase leading-none", children: order.machine }) }), _jsxs("td", { className: "px-3 py-1.5 leading-tight", children: [_jsx("p", { className: "font-bold text-slate-800 truncate max-w-[220px]", children: order.itemDescription }), _jsx("span", { className: "text-[9px] text-slate-400 font-mono", children: order.itemCode })] }), _jsx("td", { className: "px-3 py-1.5 text-center", children: _jsx("div", { className: "flex flex-col items-center gap-0.5 leading-tight", children: importMode === "smart_update" && changeMeta?.isExisting && changeMeta?.deliveryDateChanged ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "px-2 py-[2px] rounded-lg border text-[10px] font-black line-through text-slate-400 bg-slate-50 border-slate-200", title: t("digitalplanning.planning_import.old_delivery_date", "Oude leverdatum"), children: changeMeta.oldDeliveryDate ? format(new Date(changeMeta.oldDeliveryDate), "dd-MM") : "-" }), _jsx("span", { className: `px-2 py-[2px] rounded-lg border text-[10px] font-black ${deliveryColor}`, title: t("digitalplanning.planning_import.new_delivery_date", "Nieuwe leverdatum"), children: deliveryMeta.dateLabel })] })) : (_jsxs(_Fragment, { children: [_jsx("span", { className: `px-2 py-[2px] rounded-lg border text-[10px] font-black ${deliveryColor}`, children: deliveryMeta.weekLabel }), _jsx("span", { className: "text-[10px] font-bold text-slate-500", children: deliveryMeta.dateLabel })] })) }) }), _jsx("td", { className: "px-2 py-1.5 text-center", children: _jsx("span", { className: "inline-block px-2 py-[2px] rounded-lg bg-slate-100 text-slate-700 text-[10px] font-black uppercase max-w-[110px] truncate leading-none", children: order.orderStatus || "-" }) }), _jsx("td", { className: "px-2 py-1.5 text-center", children: importMode === "smart_update" && changeMeta?.isExisting && changeMeta?.quantityChanged ? (_jsxs("div", { className: "inline-flex items-center gap-1", children: [_jsx("span", { className: "text-[10px] font-black text-slate-400 line-through", children: changeMeta.oldQuantity }), _jsx("span", { className: `text-[11px] font-black px-1.5 py-[1px] rounded border ${isQtyIncrease
                                                                                ? "text-emerald-700 bg-emerald-100 border-emerald-200"
                                                                                : isQtyDecrease
                                                                                    ? "text-red-700 bg-red-100 border-red-200"
                                                                                    : "text-slate-700 bg-slate-100 border-slate-200"}`, children: changeMeta.newQuantity })] })) : (_jsx("span", { className: "text-[11px] font-black text-slate-700", children: Number(order.quantity || 0) })) }), _jsx("td", { className: "px-2 py-1.5 text-center", children: _jsx("input", { type: "number", min: "0", step: "1", value: toDoOverrides[order.id] ?? Number(order.toDoQty ?? order.plan ?? order.quantity ?? 0), onChange: (e) => {
                                                                        const nextVal = e.target.value;
                                                                        setToDoOverrides((prev) => ({
                                                                            ...prev,
                                                                            [order.id]: nextVal,
                                                                        }));
                                                                    }, className: "w-20 rounded-lg border border-slate-200 bg-white px-2 py-1 text-center text-[11px] font-black text-slate-700 outline-none focus:border-blue-500", title: t("digitalplanning.planning_import.todo_edit_title", "Pas te maken aantal aan voor import") }) }), _jsx("td", { className: "px-2 py-1.5 text-center whitespace-nowrap", children: changeMeta?.isExisting ? (_jsxs("div", { className: "inline-flex items-center gap-1 flex-col", children: [_jsxs("span", { className: "text-[10px] font-black text-slate-400 cursor-help", title: `Gemaakt: ${Number(getExistingOrder(order)?.produced || 0)}, Bij Eindinspectie: ${Number(getExistingOrder(order)?.atEindinspectieCount || 0)}`, children: ["FF ", Number(changeMeta.oldReadyQty || 0)] }), _jsxs("span", { className: `text-[10px] font-black px-1.5 py-[1px] rounded border ${readyUp
                                                                                ? "text-emerald-700 bg-emerald-100 border-emerald-200"
                                                                                : readyDown
                                                                                    ? "text-red-700 bg-red-100 border-red-200"
                                                                                    : readyEqual
                                                                                        ? "text-slate-700 bg-slate-100 border-slate-200"
                                                                                        : "text-blue-700 bg-blue-100 border-blue-200"}`, children: ["LN ", Number(changeMeta.newReadyQty || 0)] })] })) : (_jsxs("span", { className: "text-[11px] font-black text-blue-700", children: ["LN ", Number(order.produced || 0)] })) }), _jsx("td", { className: "px-2 py-1.5 text-center w-[100px]", children: order.extraCode ? (_jsx("span", { className: "inline-block max-w-[88px] truncate text-[10px] bg-amber-50 text-amber-700 px-1.5 py-[2px] rounded font-black border border-amber-100 leading-none", children: order.extraCode })) : (_jsx("span", { className: "text-[10px] text-slate-300 font-black", children: "-" })) }), _jsx("td", { className: "px-2 py-1.5 leading-tight", children: order.notes ? (_jsx("span", { className: `inline-block max-w-[200px] truncate text-[10px] px-1.5 py-[2px] rounded font-black border leading-none ${importMode === "smart_update" && changeMeta?.isExisting && changeMeta?.notesChanged
                                                                        ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                                                                        : "bg-indigo-50 text-indigo-700 border-indigo-100"}`, title: importMode === "smart_update" && changeMeta?.isExisting && changeMeta?.notesChanged ? `${t("digitalplanning.planning_import.was", "Was")}: ${changeMeta.oldNotes || "-"}` : undefined, children: order.notes })) : (_jsx("span", { className: "text-[10px] text-slate-300 font-black", children: "-" })) }), _jsx("td", { className: "px-3 py-1.5 text-center whitespace-nowrap", children: _jsxs("span", { className: "text-sm font-black text-blue-600", children: [Number(order.totalPlannedHours).toFixed(1), "h"] }) }), _jsx("td", { className: "px-3 py-1.5 text-right pr-4", children: _jsxs("label", { className: `inline-flex items-center gap-2 select-none ${isSmartUnchangedExisting ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`, children: [_jsx("input", { type: "checkbox", checked: selectedOrderIds.has(order.id), disabled: isSmartUnchangedExisting, onChange: () => {
                                                                                if (isSmartUnchangedExisting)
                                                                                    return;
                                                                                toggleOrderSelection(order.id);
                                                                            }, className: "h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" }), isExisting ? (importMode === "smart_update" && changeMeta?.hasSmartChange
                                                                            ? _jsx("span", { className: "text-emerald-600 font-black uppercase text-[10px]", children: t("digitalplanning.planning_import.sync_label", "Sync") })
                                                                            : importMode === "smart_update" && hoursOnlyMode
                                                                                ? _jsx("span", { className: "text-emerald-600 font-black uppercase text-[10px]", children: t("digitalplanning.planning_import.sync_label", "Sync") })
                                                                                : importMode === "smart_update"
                                                                                    ? _jsx("span", { className: "text-slate-400 font-black uppercase text-[10px]", children: "-" })
                                                                                    : _jsx("span", { className: "text-amber-500 font-black uppercase text-[10px]", children: t("digitalplanning.planning_import.update_label", "Update") })) : (_jsx("span", { className: "text-blue-500 font-black uppercase text-[10px]", children: t("digitalplanning.planning_import.new_label", "Nieuw") }))] }) })] }, order.id));
                                                }) })] }) })] })) }) }), _jsxs("div", { className: "p-10 border-t bg-slate-50 flex justify-between items-center relative", children: [_jsxs("div", { className: "flex gap-3 bg-white p-1.5 rounded-3xl border border-slate-200", children: [_jsx("button", { onClick: () => setImportMode("smart_update"), className: `px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${importMode === "smart_update" ? "bg-emerald-600 text-white shadow-xl" : "text-slate-400 hover:bg-slate-50"}`, children: t("digitalplanning.planning_import.smart_update", "Slimme Sync") }), _jsx("button", { onClick: () => setImportMode("overwrite"), className: `px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${importMode === "overwrite" ? "bg-orange-600 text-white shadow-xl" : "text-slate-400 hover:bg-slate-50"}`, children: t("digitalplanning.planning_import.overwrite_all", "Overschrijf Alles") })] }), _jsxs("div", { className: "flex gap-5", children: [_jsx("button", { onClick: onClose, disabled: importing, className: "px-10 py-4 bg-white border-2 border-slate-200 text-slate-400 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-100 transition-all disabled:opacity-60 disabled:cursor-not-allowed", children: t("digitalplanning.planning_import.cancel", "Annuleren") }), _jsxs("button", { onClick: startImport, disabled: importableCount === 0 || importing, className: "px-12 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-sm tracking-widest shadow-2xl shadow-blue-200 hover:bg-blue-700 transition-all flex items-center gap-4 disabled:opacity-50 disabled:bg-slate-300", children: [importing ? _jsx(Loader2, { className: "animate-spin", size: 24 }) : _jsx(ShieldCheck, { size: 24 }), importing
