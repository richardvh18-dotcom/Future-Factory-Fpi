...
import { getISOWeek, getISOWeekYear, addWeeks, subWeeks, subDays, } from "date-fns";
...
const Terminal = ({ initialStation, onCancelProduction, orders = [] }) => {
    ...
    const isDefinitiveRejectedOrRemoved = (product) => {
        ...
        const isRejected = (["rejected", "afkeur", "archived_rejected", "definitieve_afkeur", "definitief_afkeur"].includes(statusNorm) ||
            stepNorm === "rejected") &&
            !isTemporaryReject;
    ...
    useEffect(() => {
        ...
        const unsubs = years.map((year) => onSnapshot(query(collection(db, ...getArchiveItemsPath(year)), where("timestamps.finished", ">=", minArchiveDate)), (snap) => {
    ...
    const madeCountMap = useMemo(() => {
        ...
        const addLot = (orderIdRaw, lotRaw) => {
            ...
            if (!orderId || !lot)
                return;
            if (!perOrderLots.has(orderId))
                perOrderLots.set(orderId, new Set());
            perOrderLots.get(orderId).add(lot);
        };
        allTracked.forEach((p) => {
            if (isDefinitiveRejectedOrRemoved(p))
                return;
    ...
    const myOrders = useMemo(() => {
        ...
        if (["BH12", "BH15", "BH17", "BH18"].includes(cleanStationId)) {
            allTracked.forEach((p) => {
                ...
                if (stationNorm !== normalizedStationId)
                    return;
        ...
        const result = allOrders.filter(o => {
            ...
            if (isWikkelToLossenSourceStation) {
                ...
                const waitingForLossenCount = allTracked.filter((p) => {
                    if (String(p?.orderId || "").trim() !== orderId)
                        return false;
                    const stationNorm = (normalizeMachine(p?.originMachine || p?.machine || p?.currentStation) || "").toUpperCase().trim();
                    if (stationNorm !== normalizedStationId)
                        return false;
            ...
            return (machineNorm === normalizedStationId ||
    ...
    const activeWikkelingen = useMemo(() => {
        const active = allTracked
            ...
            if (currentNorm)
                return currentNorm === normalizedStationId;
    ...
    const repairItems = useMemo(() => {
        return activeWikkelingen.filter(p => p.isManualMove ||
    ...
    const filteredOrders = useMemo(() => {
        ...
        const base = enrichedOrders.filter((o) => {
            ...
            if (isLossen1218Station) {
                if (!hasActiveTracked && isInactivePlanningStatus(o.status))
                    return false;
                if (!hasActiveTracked && isFullyProduced)
                    return false;
                // Verberg Lossen 12/18 orders die absoluut inactief zijn (cancelled, deleted)
                const statusLower = String(o.status || "").toLowerCase();
                if (["cancelled", "deleted", "shipped"].includes(statusLower))
                    return false;
                return true;
            }
            ...
            if (!isBM01 && remainingAtOrder <= 0 && (stationWorkCompleted || isFullyProduced) && !hasActiveTracked && !hasStationActivity)
                return false;
            ...
            if (isFullyProduced && !hasActiveTracked && !hasStationActivity)
                return false;
            ...
            if (isRepairStation && !sidebarSearch) {
                ...
                if (started > 0 || ["in_progress", "in production", "in productie"].includes(String(o.status || "").trim().toLowerCase()))
                    return false;
            }
            // BM01: Geen week filter, toon alles (behalve als search actief is, wat hieronder gebeurt)
            if (isBM01)
                return true;
            if (showAllWeeks || sidebarSearch)
                return true;
            // FORCEER ZICHTBAARHEID ALS ER ACTIVITEIT IS (ZELFS ALS WEEK NIET MATCHT)
            if (hasStationActivity || hasActiveTracked)
                return true;
            ...
            if (absTarget === absCurrentReal) {
                if (normalizePlanningStatus(o.status) === "in_progress" || normalizePlanningStatus(o.status) === "in production")
                    return true;
                if (absOrder === absTarget)
                    return true;
                if (absOrder < absTarget)
                    return true;
            }
            ...
            return base.sort((a, b) => {
                const priorityRank = (order) => {
                    ...
                    if (normalizedPriority === "immediate")
                        return 3;
                    if (normalizedPriority === "urgent" || order?.isUrgent)
                        return 2;
                    if (normalizedPriority === "high" || order?.isMoved)
                        return 1;
                    return 0;
                };
                ...
                if (isPlannedA !== isPlannedB)
                    return isPlannedA ? -1 : 1;
                // 3. Urgentie (legacy fallback)
                if (a.isUrgent !== b.isUrgent)
                    return a.isUrgent ? -1 : 1;
                // 4. Week (Oplopend)
                if (absOrderA !== absOrderB)
                    return absOrderA - absOrderB;
                // 5. Order ID
    ...
    const selectedOrder = useMemo(() => {
        ...
        const fromPlanning = planningSource.find((o) => o.id === selectedOrderId || o.orderId === selectedOrderId);
        if (fromPlanning)
            return fromPlanning;
        // Fallback voor gevallen waarin selectie nog bestaat maar tijdelijk niet in de zichtbare lijst zit.
        return myOrders.find((o) => o.id === selectedOrderId || o.orderId === selectedOrderId);
    ...
    const handleScan = (e) => {
        if (e.key === 'Enter') {
            ...
            if (codeUpper === QR_CODE_OK_CONFIRMATION) {
                ...
                if (!isBH18) {
                    notify(t("digitalplanning.terminal.ok_qr_not_available", "OK-QR is op dit station niet beschikbaar. Gebruik deze alleen op BH18 (Wikkelen) en in Nabewerken/BM01."));
                    setScanInput("");
                    setTimeout(() => {
                        scanInputRef.current?.focus();
                    }, 50);
                ...
                if (selectedWikkeling && isWikkelenStep) {
                    ...
                    notify(t("digitalplanning.terminal.select_active_bh18_before_qr", "Selecteer eerst een actief BH18-item in stap Wikkelen voordat je de OK-QR scant."));
                    setScanInput("");
                    setTimeout(() => {
                        scanInputRef.current?.focus();
                    }, 50);
            ...
            const found = activeWikkelingen.find(i => (i.lotNumber || "").toLowerCase() === code.toLowerCase() ||
                (i.orderId || "").toLowerCase() === code.toLowerCase());
            const normalizedCode = code.toUpperCase();
            const lotMatches = activeWikkelingen.filter((i) => String(i.lotNumber || "").toUpperCase() === normalizedCode);
            const conflictOnScannedLot = lotConflictMeta[normalizedCode]?.hasConflict;
            if (lotMatches.length > 1 && conflictOnScannedLot) {
                notify(t("digitalplanning.terminal.lot_duplicate_conflict", "Lot {{code}} bestaat meerdere keren met verschillend product/order. Kies handmatig het juiste item in de lijst.", { code }));
                setScanInput("");
                setTimeout(() => {
                    scanInputRef.current?.focus();
                }, 50);
            ...
            if (found) {
                ...
                notify(t("digitalplanning.terminal.item_not_found_active_winding", "Item {{code}} niet gevonden in actieve wikkelingen.", { code }));
                setScanInput("");
            }
    ...
    const handleStartProduction = async (order, lot, _stringCount, _manualOrderInput, _operatorInput, _selectedOperatorName, labelZplData, labelTemplateId, startOptions = {}) => {
        ...
        try {
            ...
            void logActivity(user?.uid || "system", "ORDER_RELEASE", `Terminal start productie: order ${order.orderId}, station ${effectiveStationId}, lots ${createdLots.join(", ")}`).catch((logError) => {
    ...
    const handleRepairComplete = async (data) => {
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "QUALITY_REPAIR_COMPLETE", `Reparatie voltooid: lot ${itemToRepair.lotNumber || itemToRepair.id}, station ${effectiveStationId}`);
    ...
    if (loading)
        return (_jsx("div", { className: "flex h-screen items-center justify-center bg-white", children: _jsx(Loader2, { className: "animate-spin text-blue-600", size: 48 }) }));
    if (isSimpleViewStation) {
        if (isNabewerking) {
            return (_jsx("div", { className: "flex-1 overflow-hidden h-full text-left", children: _jsx(Nabewerken, { products: allTracked, orders: orders }) }));
        }
        if (isMazak) {
            return (_jsx("div", { className: "flex flex-col h-full bg-slate-50 text-slate-900 overflow-hidden animate-in fade-in", children: _jsx("div", { className: "flex-1 overflow-hidden h-full text-left", children: _jsx(MazakView, { stationId: effectiveStationId, products: allTracked }) }) }));
        }
        return (_jsx("div", { className: "flex flex-col h-full bg-slate-50 text-slate-900 overflow-hidden animate-in fade-in", children: _jsx("div", { className: "flex-1 overflow-hidden h-full text-left", children: _jsx(LossenView, { stationId: effectiveStationId, appId: appId, products: allTracked }) }) }));
    }
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex flex-col h-full bg-slate-50 text-slate-900 overflow-hidden animate-in fade-in", children: [_jsx("div", { className: "p-2 bg-white border-b border-slate-200 shrink-0 shadow-sm text-left", children: _jsxs("div", { className: "flex items-center justify-center relative", children: [_jsx("div", { className: "flex bg-slate-100 p-1 rounded-2xl w-full max-w-xl", children: (() => {
                                        ...
                                        return tabLabels.map((tabLabel, idx) => {
                                            ...
                                            return (_jsx("button", { onClick: () => handleTabChange(tabKey), className: `flex-1 px-4 md:px-6 py-2 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tabKey ? "bg-white shadow-sm text-blue-600" : "text-slate-500 hover:text-slate-700"}`, children: tabLabel }, tabKey));
                                        });
                                    })() }), activeTab === "wikkelen" && (_jsxs("button", { onClick: () => setScannerMode(!scannerMode), className: `absolute right-0 md:right-4 top-1/2 -translate-y-1/2 flex items-center gap-2 px-3 py-2 rounded-lg border-2 font-bold text-[10px] uppercase tracking-widest transition-all ${scannerMode ? 'bg-purple-100 border-purple-200 text-purple-700' : 'bg-white border-slate-200 text-slate-400'}`, title: scannerMode ? t("digitalplanning.terminal.scanner_keyboard_hidden", "Toetsenbord verborgen (Scanner modus)") : t("digitalplanning.terminal.normal_input", "Normale invoer"), children: [scannerMode ? _jsx(ScanBarcode, { size: 16 }) : _jsx(Keyboard, { size: 16 }), _jsx("span", { className: "hidden sm:inline", children: scannerMode ? t("digitalplanning.terminal.scanner", "Scanner") : t("digitalplanning.terminal.keyboard", "Toetsenbord") })] }))] }) }), _jsx("div", { className: "flex-1 overflow-hidden flex flex-col", children: 
                        /* STANDAARD PLANNING & WIKKELEN FLOW */
                        _jsx("div", { className: "flex-1 overflow-hidden flex flex-col lg:flex-row text-left", children: activeTab === "planning" && isLossen1218Station ? (
                            /* LOSSEN 12/18: Volledige planning van BH12/15/17/18 met machinefilter */
                            _jsxs("div", { className: "flex-1 overflow-hidden flex flex-col", children: [_jsxs("div", { className: "flex items-center gap-2 px-3 pt-2 pb-2 bg-white border-b border-slate-100 shrink-0 flex-wrap", children: [[null, "BH12", "BH15", "BH17", "BH18"].map(f => (_jsx("button", { onClick: () => setLossenPlanningFilter(f), className: `px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${lossenPlanningFilter === f
                                                    ? "bg-blue-600 text-white border-blue-600"
                                                    : "bg-white text-slate-500 border-slate-200 hover:border-blue-300"}`, children: f ?? t("common.all", "Alles") }, f ?? "all"))), _jsxs("span", { className: "text-[10px] text-slate-400 font-bold uppercase tracking-widest ml-auto", children: [lossenFilteredOrders.length, " ", t("digitalplanning.terminal.orders", "orders")] })] }), _jsx("div", { className: "flex-1 overflow-hidden flex lg:flex-row", children: _jsx(TerminalPlanningView, { orders: lossenFilteredOrders, selectedOrderId: selectedOrderId, onSelectOrder: setSelectedOrderId, searchTerm: sidebarSearch, onSearchChange: setSidebarSearch, referenceDate: referenceDate, onDateChange: (direction) => {
                                                if (direction === 'reset')
                                                    setReferenceDate(new Date());
                                                else
                                                    setReferenceDate(direction === 'prev' ? subWeeks(referenceDate, 1) : addWeeks(referenceDate, 1));
                                            }, showAllWeeks: showAllWeeks, onToggleAllWeeks: () => setShowAllWeeks(!showAllWeeks), targetWeekNum: targetWeekNum, productionProgressMap: productionProgressMap, rejectedCountMap: rejectedCountMap, readyForReturnMap: readyForReturnMap, isBM01: false, trackedProducts: allTracked, onStartProduction: null, selectedOrder: selectedOrder, onViewDrawing: handleViewDrawing, repairItems: [], onRepair: null, optimizationPanel: _jsx(MalOptimizationPanel, { currentOrder: selectedOrder, allOrders: myOrders, onSelectOrder: setSelectedOrderId }) }) })] })) : activeTab === "planning" ? (_jsx(TerminalPlanningView, { orders: filteredOrders, selectedOrderId: selectedOrderId, onSelectOrder: setSelectedOrderId, searchTerm: sidebarSearch, onSearchChange: setSidebarSearch, referenceDate: referenceDate, onDateChange: (direction) => {
                                    if (direction === 'reset')
                                        setReferenceDate(new Date());
                                    else
                                        setReferenceDate(direction === 'prev' ? subWeeks(referenceDate, 1) : addWeeks(referenceDate, 1));
                                }, showAllWeeks: showAllWeeks, onToggleAllWeeks: () => setShowAllWeeks(!showAllWeeks), targetWeekNum: targetWeekNum, productionProgressMap: productionProgressMap, rejectedCountMap: rejectedCountMap, readyForReturnMap: readyForReturnMap, isBM01: isBM01, trackedProducts: allTracked, onStartProduction: () => setShowStartModal(true), selectedOrder: selectedOrder, onViewDrawing: handleViewDrawing, repairItems: repairItems, onRepair: handleRepair, 
                                // Mal Optimalisatie: Toon gerelateerde orders in het paneel
                                optimizationPanel: _jsx(MalOptimizationPanel, { currentOrder: selectedOrder, allOrders: myOrders, onSelectOrder: setSelectedOrderId }) })) : activeTab === "wikkelen" ? (
                            /* TAB WIKKELEN */
                            _jsx(TerminalProductionView, { activeWikkelingen: activeWikkelingen, lotConflictMeta: lotConflictMeta, selectedTrackedId: selectedTrackedId, onSelectTracked: setSelectedTrackedId, selectedWikkeling: selectedWikkeling, onReleaseProduct: handleOpenReleaseModal, scanInput: scanInput, setScanInput: setScanInput, onScan: handleScan, scanInputRef: scanInputRef, scannerMode: scannerMode, onCancelProduction: onCancelProduction, activeTab: activeTab })) : activeTab === "gereed" ? (_jsx(TerminalGereedTab, { allTracked: allTracked, stationId: stationId, effectiveStationId: effectiveStationId })) : (
                            /* TAB LOSSEN */
                            _jsx("div", { className: "flex-1 overflow-hidden h-full text-left", children: isMazak ? (_jsx(MazakView, { stationId: effectiveStationId, products: allTracked })) : (_jsx(LossenView, { stationId: effectiveStationId, appId: appId, products: allTracked })) })) }) })] }), _jsx(TerminalManualInput, { isOpen: showManualInput, onClose: () => setShowManualInput(false), value: manualInputValue, onChange: setManualInputValue, onSearch: () => {
                    setSidebarSearch(manualInputValue);
                    setShowManualInput(false);
                } }), showStartModal && selectedOrder && (_jsx("div", { className: "fixed z-[9999]", children: _jsx(ProductionStartModal, { isOpen: true, onClose: () => setShowStartModal(false), order: selectedOrder, stationId: stationId, onStartInitiated: () => {
                        setShowStartModal(false);
                        if (!isNabewerking && !isLossenStation && !isBM01 && !isBH31) {
                            setActiveTab("wikkelen");
                        }
                    }, onStart: handleStartProduction, existingProducts: allTracked }) })), productToRelease && (_jsx("div", { className: "fixed z-[9999]", children: _jsx(ProductReleaseModal, { isOpen: true, product: productToRelease, bulkProducts: bulkProductsToRelease, autoApproveTrigger: releaseAutoApproveToken, onClose: () => {
                        setProductToRelease(null);
                        setBulkProductsToRelease([]);
                        setSelectedTrackedId(null);
                    }, appId: appId }) })), showRepairModal && itemToRepair && (_jsx("div", { className: "fixed z-[9999]", children: _jsx(RepairModal, { product: itemToRepair, onClose: () => { setShowRepairModal(false); setItemToRepair(null); }, onConfirm: handleRepairComplete }) })), viewingProduct && (_jsx("div", { className: "fixed z-[9999]", children: _jsx(ProductDetailModal, { product: viewingProduct, onClose: () => setViewingProduct(null), userRole: user?.role || "operator" }) }))] }));
};
