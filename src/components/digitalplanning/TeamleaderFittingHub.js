...
const TeamleaderFittingHub = (props) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
            ...
            if (docSnap.exists()) {
                ...
                const myDept = (data.departments || []).find((d) => d.slug === "fittings" || d.id === "fittings");
                if (myDept) {
    ...
    if (loading)
        return (_jsx("div", { className: "h-full flex items-center justify-center bg-slate-50", children: _jsx(Loader2, { className: "animate-spin text-blue-600", size: 32 }) }));
    const machineIds = stations.map((s) => s.name);
    return (_jsx(TeamleaderHub, { ...props, fixedScope: "fittings", departmentName: t('teamleader.fitting_productions', 'Fitting Producties'), allowedMachines: machineIds }));
};
