...
import { moveTrackedProductManual, archiveRejectedTrackedProduct, assignOverproduction, createPlanningOrderManual, updatePlanningOrderDetails, saveOccupancyAssignments, deleteOccupancyAssignments, } from "../../services/planningSecurityService";
...
// User & context
user, navigate, t, todayStr, 
// State setters
setActiveTab, setIsMobileMenuOpen, setShowAiPrediction, setIsSyncingDrawings, setModalTitle, setKpiWeekOffset, setActiveKpi, setLastKpi, setSelectedSidebarEntry, setSelectedOrderId, setViewingDossier, setSelectedStationDetail, setSelectedOverproductionGroup, setOverproductionTargetOrderId, setOverproductionManualStation, setAssigningOverproduction, setIsCopying, setIsClearing, setShowAddOrderModal, setCreatingOrder, setNewOrderData, setIsArchivingLegacyRejected, 
// Notifications
showSuccess, showInfo, showWarning, showConfirm, notify, 
// Data
dataStore, rawOrders, rawProducts, bezetting, selectedOverproductionGroup, overproductionTargetOrderId, overproductionManualStation, selectedSidebarEntry, newOrderData, legacyRejectedOrders, 
// Helpers
getOrderIdFromTrackedRecord, getOrderProgressMeta, getFinishedQtyForOrder, resolveOverproductionRoute, isInAllowedScope, fixedScope, targetSlug, departmentFilter, effectiveAllowedNorms = [], }) => {
    // Navigation handler
    ...
    const handleAssignOverproduction = useCallback(async () => {
        if (!selectedOverproductionGroup)
            return;
        ...
        if (!targetOrder?.id) {
            showWarning(t('teamleader.order_not_visible_yet', 'Order {{orderId}} is not visible in planning yet. Import or sync the LN order first.', {
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "OVERPRODUCTION_ASSIGN", t('teamleader.overproduction_linked_log', 'Overproduction linked: {{count}} pieces from {{sourceOrderId}} -> {{targetOrderId}}, station {{station}}', {
            ...
            showWarning(t('teamleader.overproduction_link_failed', 'Linking failed: {{message}}', {
                message: err.message,
            }));
        }
        finally {
    ...
    const handleSidebarSelect = useCallback(async (entry) => {
        if (!entry) {
    ...
    const handleOpenArchivedLotDossier = useCallback(async (lotNumber) => {
    ...
    const handleReopenArchivedOrderWithIncrease = useCallback(async ({ entry, increaseBy }) => {
        ...
        const orderRef = String(targetEntry.__docPath ||
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "PLANNING_REOPEN_FROM_ARCHIVE", `Order ${orderLabel} heropend uit archief en opgehoogd met ${safeIncrease}`);
    ...
    // KPI modal handler
    const handleKpiClick = useCallback((kpiId, label) => {
        setModalTitle(label);
        setKpiWeekOffset(0);
        setActiveKpi(kpiId);
    ...
    const handleCopyYesterday = useCallback(async (targetDeptId) => {
        const yesterdayStr = format(subDays(new Date(), 1), "yyyy-MM-dd");
        const currentDayStr = format(new Date(), "yyyy-MM-dd");
        const yesterdayData = bezetting.filter((o) => o.date === yesterdayStr &&
            o.operatorNumber &&
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "OCCUPANCY_COPY_YESTERDAY", `Bezetting gekopieerd van gisteren: afdeling ${targetDeptId}, aantal ${yesterdayData.length}`);
        }
        catch (err) {
    ...
    const handleClearToday = useCallback(async (targetDeptId) => {
        const currentDayStr = format(new Date(), "yyyy-MM-dd");
        const todayData = bezetting.filter((o) => o.date === currentDayStr && o.departmentId === targetDeptId);
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "OCCUPANCY_CLEAR_TODAY", `Bezetting gewist voor vandaag: afdeling ${targetDeptId}, aantal ${todayData.length}`);
        }
        catch (err) {
    ...
    const handleMoveLot = useCallback(async (lotNumber, newStation, options = {}) => {
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "LOT_MANUAL_MOVE", `${isRepairMove ? "Teamleader reparatie" : "Teamleader verplaatsing"}: lot ${lotNumber} -> ${newStation}${repairInstruction ? ` | instructie: ${repairInstruction}` : ""}`);
    ...
    const handleArchiveRejectedProduct = useCallback(async (product) => {
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "QUALITY_REJECT_ARCHIVE", `Definitieve afkeur afgesloten: ${product?.lotNumber || productId}`);
    ...
    const handleCreateOrder = useCallback(async (e) => {
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "ORDER_CREATE_MANUAL", `Teamleader order aangemaakt: ${newOrderData.orderId}, machine ${newOrderData.machine}, plan ${newOrderData.plan}`);
    ...
    const handleArchiveLegacyRejectedOrders = useCallback(async () => {
        ...
        try {
            ...
            await logActivity(user?.uid || "system", "PLANNING_ARCHIVE_LEGACY_REJECTED", `Oude definitieve afkeur-orders gearchiveerd: ${archivedCount}/${legacyRejectedOrders.length}`);
            if (archivedCount > 0) {
            ...
            if (archivedCount !== legacyRejectedOrders.length) {
                showWarning(`Waarschuwing: ${legacyRejectedOrders.length - archivedCount} order(s) konden niet gearchiveerd worden.`);
            }
        }
        catch (err) {
