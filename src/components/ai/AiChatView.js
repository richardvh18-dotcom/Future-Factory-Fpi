...
const AiChatView = () => {
    ...
    const analyzeDocument = async (text, fileName) => {
        ...
        try {
            ...
            if (!jsonText)
                return { parsed: false, analysis: { title: fileName, summary: response.substring(0, 1000), tags: ["niet-geparsed"], fullContext: response.substring(0, 10000) } };
    ...
    const handleSendChat = async (e, queryOverride = null) => {
        ...
        try {
            if (!aiService.isConfigured())
                throw new Error('Google AI API key niet gevonden.');
            ...
            try {
                    ...
                    efficiencyRows.forEach(std => {
                        if (std.orderId) {
                            ...
                            relatedLogs.forEach(log => {
                                ...
                                if (log.status === 'completed' || log.currentStep === 'Finished' || log.currentStation === 'GEREED')
                                    producedQty++;
                            });
                            ...
                            if (remainingQty <= 0 && producedQty > 0)
                                estimatedEnd = "Reeds voltooid";
                ...
                console.error("Kon efficiency niet ophalen:", err);
            }
            try {
            ...
            try {
                ...
                console.error("Kon documenten niet ophalen:", err);
            }
    ...
    if (isInitializing) {
        return (_jsxs("div", { className: "flex flex-col h-full bg-slate-50 items-center justify-center animate-in fade-in", children: [_jsx("div", { className: "w-12 h-12 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin mb-4" }), _jsx("h2", { className: "text-lg font-bold text-slate-700", children: "AI Assistent Starten..." }), _jsx("p", { className: "text-slate-500 text-sm", children: "Even geduld, we laden de context." })] }));
    }
    return (_jsxs("div", { className: "h-full flex flex-col max-w-4xl mx-auto", children: [_jsxs("div", { className: "flex items-center justify-between px-6 py-2 border-b border-slate-200 bg-white shrink-0", children: [_jsx("span", { className: "text-xs text-slate-400 select-none", children: "AI Assistent" }), _jsx("button", { onClick: handleNewConversation, className: "text-xs text-slate-500 hover:text-red-600 transition-colors", title: "Nieuw gesprek starten", children: "Nieuw gesprek" })] }), _jsxs("div", { className: "flex-1 overflow-y-auto p-6 space-y-4", children: [messages.map((msg, idx) => (_jsx(AiMessage, { message: msg, factoryStructure: factoryStructure, onNavigate: navigate, onQuery: (text) => handleSendChat(null, text), onLike: msg.role === "assistant" && idx > 0 ? async () => {
                        ...
                        } }, idx))), _jsx("div", { ref: messagesEndRef })] }), _jsx("div", { className: "p-4 bg-white border-t border-slate-200", children: _jsxs("form", { onSubmit: handleSendChat, className: "flex gap-2", children: [_jsx("input", { type: "text", className: "flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-blue-500 transition-colors", placeholder: "Typ je vraag...", value: input, onChange: (e) => setInput(e.target.value), disabled: isLoading || isUploadingDocument }), _jsxs("label", { className: `inline-flex items-center justify-center p-3 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 transition-colors cursor-pointer ${isUploadingDocument ? "opacity-50 cursor-not-allowed" : ""}`, title: "Upload document (PDF of tekst)", children: [_jsx(Paperclip, { size: 18 }), _jsx("input", { type: "file", accept: ".pdf,.txt,.md,.csv,.json", onChange: handleChatFileUpload, disabled: isUploadingDocument, className: "hidden" })] }), _jsx("button", { type: "button", onClick: handleStopChat, disabled: !isLoading, className: "bg-white text-slate-700 border border-slate-200 p-3 rounded-xl hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed", title: "Stop antwoord", children: "Stop" }), _jsx("button", { type: "submit", disabled: isLoading, className: "bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed", children: isLoading ? _jsx("div", { className: "w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" }) : _jsx(Send, { size: 20 }) })] }) })] }));
};
