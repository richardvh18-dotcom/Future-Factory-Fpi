...
import { Bot, MessageSquare, BookOpen, Download, Loader2, } from "lucide-react";
...
const AiAssistantView = () => {
    ...
    const handleExportExcel = async () => {
        try {
            if (showInfo)
                showInfo(t('ai.export.fetching', 'Planning data ophalen...'));
    ...
    return (_jsxs("div", { className: "flex flex-col h-full bg-slate-50 animate-in fade-in", children: [_jsxs("div", { className: "px-6 py-4 bg-white border-b border-slate-200 flex justify-between items-center shrink-0", children: [_jsxs("div", { children: [_jsxs("h1", { className: "text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3", children: [_jsx(Bot, { className: "text-blue-600" }), t('ai.title', 'AI Assistent')] }), _jsx("p", { className: "text-slate-500 font-medium text-sm mt-1", children: t('ai.subtitle', 'Stel vragen, vraag om uitleg, of start een trainingssessie.') }), _jsx("p", { className: "text-xs text-orange-500 mt-1 font-medium", children: t('ai.disclaimer', 'AI is aan het leren, fouten kunnen voorkomen.') })] }), _jsxs("div", { className: "flex bg-slate-100 p-1 rounded-xl gap-1", children: [_jsxs("button", { onClick: () => setActiveTab("chat"), className: `px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === "chat"
                                    ? "bg-white text-blue-600 shadow-sm"
                                    : "text-slate-500 hover:text-slate-700"}`, children: [_jsx(MessageSquare, { size: 16 }), " ", t('ai.tabs.chat', 'Chat')] }), _jsxs("button", { onClick: () => setActiveTab("flashcards"), className: `px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === "flashcards"
                                    ? "bg-white text-purple-600 shadow-sm"
                                    : "text-slate-500 hover:text-slate-700"}`, children: [_jsx(BookOpen, { size: 16 }), " ", t('ai.tabs.flashcards', 'Kaartjes')] }), _jsx("button", { onClick: handleExportExcel, className: "px-3 py-2 rounded-lg text-slate-500 hover:text-green-600 hover:bg-white transition-all border border-transparent hover:border-slate-200", title: t('ai.export.tooltip', 'Exporteer huidige planning naar Excel'), children: _jsx(Download, { size: 18 }) })] })] }), _jsxs("div", { className: `flex-1 relative ${activeTab === 'chat' ? 'overflow-hidden' : 'overflow-y-auto'}`, children: [activeTab === "chat" && _jsx(AiChatView, {}), activeTab === "flashcards" && (loadingFlashcards ? (_jsxs("div", { className: "flex flex-col items-center justify-center h-full gap-4", children: [_jsx(Loader2, { className: "animate-spin text-purple-500", size: 40 }), _jsx("p", { className: "text-xs text-slate-500 font-medium", children: "Flashcards laden..." })] })) : (_jsx(FlashcardViewer, { data: flashcards, onClose: () => setActiveTab("chat") })))] })] }));
};
