...
import { FileText, TrendingUp, Clock, Users, Package, Activity, Download, Filter, BarChart3, LineChart, FileSpreadsheet, Loader2, CheckCircle2, XCircle, AlertTriangle, Box, Factory, Target, Zap, } from "lucide-react";
...
const AdminReportsView = () => {
    ...
    const factoryDepartmentMeta = useMemo(() => {
        const normalizeDeptLabel = (value) => String(value || "")
            .replace(/\u00A0/g, " ")
            .replace(/\s+/g, " ")
            .trim()
            .toLowerCase();
        ...
        byLabelList.forEach((entry) => {
            ...
            if (normalizedLabel)
                normalizedLabelToLabel[normalizedLabel] = entry.label;
    ...
    const reportCategories = [
        {
            ...
            icon: _jsx(Factory, { size: 24, className: "text-blue-600" }),
        ...
        },
        {
            ...
            icon: _jsx(CheckCircle2, { size: 24, className: "text-green-600" }),
        ...
        },
        {
            ...
            icon: _jsx(Users, { size: 24, className: "text-orange-600" }),
        ...
        },
        {
            ...
            icon: _jsx(Box, { size: 24, className: "text-teal-600" }),
        ...
        },
        {
            ...
            icon: _jsx(FileSpreadsheet, { size: 24, className: "text-indigo-600" }),
    ...
    const getDepartmentLabel = (item) => {
        ...
        const machineCandidate = item?.originMachine ||
            item?.machine ||
            item?.currentStation ||
            item?.lastStation ||
            "";
        ...
        if (exact)
            return exact.label;
        const byContains = factoryDepartmentMeta.list.find((d) => rawDepartment && (d.key.includes(rawDepartment) || rawDepartment.includes(d.key)));
        if (byContains)
            return byContains.label;
        const findByKeyword = (keyword) => factoryDepartmentMeta.byLabelList.find((d) => d.label.toLowerCase().includes(keyword))?.label;
        if (rawDepartment.includes("pipe"))
            return findByKeyword("pipe") || "Pipes";
        if (rawDepartment.includes("fit"))
            return findByKeyword("fit") || "Fittings";
        if (rawDepartment.includes("spool"))
            return findByKeyword("spool") || "Spools";
    ...
    const getDepartmentDisplayLabel = (deptKey) => {
        ...
        if (factoryDepartmentMeta.keyToLabel[key])
            return factoryDepartmentMeta.keyToLabel[key];
        if (factoryDepartmentMeta.normalizedLabelToLabel[key])
            return factoryDepartmentMeta.normalizedLabelToLabel[key];
        if (key === "onbekend")
            return "Onbekend";
        return raw;
    };
    ...
    const isDepartmentScopedReport = (reportId) => ["production_output", "lead_time", "order_completion", "wip_status"].includes(reportId);
    ...
    const isOfferedToInspection = (item) => {
        ...
        return (status.includes("te keuren") ||
            station.includes("BM01") ||
            step.includes("EINDINSPECTIE") ||
            !!item?.timestamps?.eindinspectie_start);
    };
    const isProducedButNotOffered = (item) => {
        if (isCompletedAtInspection(item) || isOfferedToInspection(item))
            return false;
        const status = String(item?.status || "").toLowerCase();
        const step = String(item?.currentStep || "").toLowerCase();
        const hasProductionSignals = !!item?.timestamps?.station_start ||
    ...
    const fetchProductionData = async () => {
        try {
            ...
            const completed = filteredProducts.filter((p) => p.status === "completed" || p.currentStep === "Finished").length;
            const inProgress = filteredProducts.filter((p) => p.status !== "completed" && p.currentStep !== "Finished" && p.status !== "rejected").length;
            const rejected = filteredProducts.filter((p) => p.status === "rejected" || p.currentStep === "REJECTED").length;
            return {
                summary: {
                ...
                chartData: Object.entries(stationCounts)
                    ...
                    .slice(0, 10),
                details: Object.entries(filteredProducts.reduce((acc, p) => {
    ...
    const fetchLeadTimeData = async () => {
        try {
            ...
            const byStation = {};
            leadTimes.forEach((x) => {
                if (!byStation[x.station])
                    byStation[x.station] = { total: 0, count: 0 };
    ...
    const fetchOrderCompletionData = async () => {
        try {
            ...
            products.forEach((p) => {
                ...
                if (p.status === "completed" || p.currentStep === "Finished")
                    byOrder[orderId].completed += 1;
            });
    ...
    const fetchQualityData = async () => {
        ...
        try {
            ...
            const filteredProducts = filters.station !== "ALL"
                ...
                : products;
            // Count rejections
            const rejections = filteredProducts.filter((p) => p.status === "rejected" || p.currentStep === "REJECTED");
            const tempRejects = filteredProducts.filter((p) => p.currentStep === "temp_reject" || p.status === "temp_reject");
            const completed = filteredProducts.filter((p) => p.status === "completed" || p.currentStep === "Finished");
    ...
    const fetchEfficiencyData = async () => {
        ...
        try {
            ...
            const occupancyQuery = query(collection(readDb, ...readPaths.OCCUPANCY), limit(500));
    ...
    const fetchTempRejectData = async () => {
        ...
        try {
            ...
            const tempRejects = products.filter((p) => {
                ...
                return (status.includes("temp_reject") ||
    ...
    const fetchMeasurementsData = async () => {
        ...
        try {
            ...
            const [trackingSnap, ...archiveSnaps] = await Promise.all([
                ...
                ...years.map((year) => getDocs(query(collection(readDb, ...getArchiveItemsPathForSource(year)), limit(4000)))),
            ]);
            const trackingProducts = trackingSnap.docs.map((d) => ({ id: d.id, source: "tracking", ...d.data() }));
            const archivedProducts = archiveSnaps.flatMap((snap) => snap.docs.map((d) => ({ id: d.id, source: "archive", ...d.data() })));
    ...
    const fetchOfferedTotalsData = async () => {
        ...
        try {
            ...
            const itemMatchesOfferedFilters = (item) => {
                ...
                if (offeredDepartmentFilter !== "ALL" && department !== offeredDepartmentFilter)
                    return false;
                if (offeredWorkstationFilter !== "ALL" && workstation !== offeredWorkstationFilter)
                    return false;
                return true;
            };
            ...
            const allWorkstations = Array.from(new Set(factoryDepartmentMeta.byLabelList.flatMap((d) => d.stations || []))).sort();
            ...
            const activeProducedNotOffered = trackingProducts.filter((p) => {
                const itemDate = getItemDate(p);
                if (itemDate && !isWithinInterval(itemDate, { start: startDate, end: endDate }))
                    return false;
            ...
            const stationOverview = Object.values(stationBuckets).sort((a, b) => b.completedAtInspection - a.completedAtInspection || b.producedNotOffered - a.producedNotOffered);
...
<html>
  ...
  <body style="font-family:Arial,sans-serif;padding:24px;">
    ...
    useEffect(() => {
        if (selectedReport?.id !== "offered_totals")
            return;
    ...
    useEffect(() => {
        if (!selectedReport?.id)
            return;
        if (!isDepartmentScopedReport(selectedReport.id))
            return;
        generateReportData(selectedReport);
    }, [productionDepartmentFilter]);
    useEffect(() => {
        if (selectedReport?.id !== "product_measurements")
            return;
        generateReportData(selectedReport);
    ...
    useEffect(() => {
        if (selectedReport?.id !== "offered_totals")
            return;
        if (!reportData?.availableDepartments?.length)
            return;
    ...
    const sourceBadge = (_jsxs("div", { className: `mb-4 inline-flex items-center rounded-xl border px-3 py-1.5 text-[11px] font-black uppercase tracking-widest ${usePilotReadData ? "border-amber-300 bg-amber-50 text-amber-800" : "border-slate-300 bg-slate-100 text-slate-700"}`, children: ["Databron: ", usePilotReadData ? "Pilot DB (Read Only)" : "Huidige DB"] }));
    // Render category selection
    if (!selectedCategory) {
        return (_jsx("div", { className: "h-full overflow-y-auto bg-slate-50 p-8", children: _jsxs("div", { className: "max-w-7xl mx-auto", children: [sourceBadge, _jsxs("div", { className: "mb-8", children: [_jsx("h2", { className: "text-3xl font-black text-slate-800 uppercase tracking-tight mb-2", children: t("reports.title", "Rapportage Centre") }), _jsx("p", { className: "text-slate-500 text-sm", children: t("reports.subtitle", "Selecteer een rapportage categorie om te beginnen") })] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6", children: reportCategories.map((category) => (_jsxs("button", { onClick: () => setSelectedCategory(category), className: `group p-8 rounded-3xl border-2 ${category.color} hover:shadow-xl transition-all duration-300 text-left active:scale-95`, children: [_jsx("div", { className: "p-4 bg-white rounded-2xl shadow-md w-fit mb-6 group-hover:scale-110 transition-transform", children: category.icon }), _jsx("h3", { className: "text-xl font-black text-slate-800 uppercase tracking-tight mb-3", children: category.title }), _jsx("p", { className: "text-xs text-slate-500 leading-relaxed mb-4", children: category.description }), _jsxs("div", { className: "flex items-center gap-2 text-xs font-bold text-slate-400", children: [category.reports.length, " rapporten beschikbaar"] })] }, category.id))) })] }) }));
    }
    // Render report selection
    if (selectedCategory && !selectedReport) {
        return (_jsx("div", { className: "h-full overflow-y-auto bg-slate-50 p-8", children: _jsxs("div", { className: "max-w-7xl mx-auto", children: [sourceBadge, _jsx("button", { onClick: () => setSelectedCategory(null), className: "mb-6 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors", children: "\u2190 Terug naar categorie\u00EBn" }), _jsx("div", { className: "mb-8", children: _jsxs("div", { className: "flex items-center gap-4 mb-3", children: [_jsx("div", { className: `p-3 rounded-2xl ${selectedCategory.color}`, children: selectedCategory.icon }), _jsxs("div", { children: [_jsx("h2", { className: "text-3xl font-black text-slate-800 uppercase tracking-tight", children: selectedCategory.title }), _jsx("p", { className: "text-slate-500 text-sm", children: selectedCategory.description })] })] }) }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6", children: selectedCategory.reports.map((report) => (_jsxs("button", { onClick: () => {
                                setSelectedReport(report);
                                generateReportData(report);
                            }, className: "group p-6 bg-white border border-slate-200 rounded-2xl hover:shadow-lg transition-all duration-300 text-left active:scale-95", children: [_jsx("div", { className: "p-3 bg-slate-50 rounded-xl w-fit mb-4 group-hover:bg-blue-50 transition-colors", children: report.icon }), _jsx("h3", { className: "text-lg font-black text-slate-800 uppercase tracking-tight mb-2", children: report.title }), _jsx("p", { className: "text-xs text-slate-500 leading-relaxed mb-4", children: report.description }), _jsxs("div", { className: "flex flex-wrap gap-1", children: [report.metrics.slice(0, 3).map((metric) => (_jsx("span", { className: "px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase rounded", children: metric }, metric))), report.metrics.length > 3 && (_jsxs("span", { className: "px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase rounded", children: ["+", report.metrics.length - 3] }))] })] }, report.id))) })] }) }));
    }
    const canExport = !!reportData && !loading;
    // Render report view with data
    return (_jsxs("div", { className: "h-full overflow-y-auto bg-slate-50", children: [_jsx("div", { className: "bg-white border-b border-slate-200 p-6 sticky top-0 z-10 shadow-sm", children: _jsxs("div", { className: "max-w-7xl mx-auto", children: [sourceBadge, _jsx("button", { onClick: () => setSelectedReport(null), className: "mb-4 px-4 py-2 bg-slate-100 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-200 transition-colors", children: "\u2190 Terug naar rapporten" }), _jsxs("div", { className: "flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4", children: [_jsxs("div", { children: [_jsx("h2", { className: "text-2xl font-black text-slate-800 uppercase tracking-tight", children: selectedReport.title }), _jsx("p", { className: "text-sm text-slate-500", children: selectedReport.description })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsxs("button", { onClick: exportToCSV, disabled: !canExport, className: "px-4 py-2 bg-green-50 text-green-700 rounded-xl text-sm font-bold hover:bg-green-100 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed", children: [_jsx(FileSpreadsheet, { size: 16 }), " CSV"] }), _jsxs("button", { onClick: exportToExcel, disabled: !canExport, className: "px-4 py-2 bg-blue-50 text-blue-700 rounded-xl text-sm font-bold hover:bg-blue-100 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed", children: [_jsx(FileSpreadsheet, { size: 16 }), " Excel"] }), _jsxs("button", { onClick: exportToPDF, disabled: !canExport, className: "px-4 py-2 bg-red-50 text-red-700 rounded-xl text-sm font-bold hover:bg-red-100 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed", children: [_jsx(FileText, { size: 16 }), " PDF"] })] })] }), _jsxs("div", { className: "mt-6 flex flex-wrap gap-3", children: [_jsxs("select", { value: dateRange, onChange: (e) => setDateRange(e.target.value), className: "px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700", children: [_jsx("option", { value: "today", children: "Vandaag" }), _jsx("option", { value: "week", children: "Deze Week" }), _jsx("option", { value: "month", children: "Deze Maand" }), _jsx("option", { value: "custom", children: "Custom Periode" })] }), selectedReport?.id !== "offered_totals" && (_jsxs("select", { value: filters.station, onChange: (e) => setFilters({ ...filters, station: e.target.value }), className: "px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700", children: [_jsx("option", { value: "ALL", children: "Alle Werkstations" }), _jsx("option", { value: "BH11", children: "BH11" }), _jsx("option", { value: "BH16", children: "BH16" }), _jsx("option", { value: "BH18", children: "BH18" }), _jsx("option", { value: "BH31", children: "BH31" }), _jsx("option", { value: "BM01", children: "BM01" })] })), _jsxs("button", { onClick: generateReportData, className: "px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 transition-colors flex items-center gap-2", children: [loading ? _jsx(Loader2, { size: 16, className: "animate-spin" }) : _jsx(Filter, { size: 16 }), loading ? "Laden..." : "Genereer Rapport"] }), isDepartmentScopedReport(selectedReport?.id) && (_jsxs("select", { value: productionDepartmentFilter, onChange: (e) => setProductionDepartmentFilter(e.target.value), className: "px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700", children: [_jsx("option", { value: "ALL", children: "Alle Afdelingen" }), factoryDepartmentMeta.byLabelList
                                            .map((d) => d.label)
                                            .sort((a, b) => a.localeCompare(b))
                                            .map((label) => (_jsx("option", { value: label, children: label }, label)))] })), selectedReport?.id === "offered_totals" && (_jsxs(_Fragment, { children: [_jsxs("select", { value: offeredDepartmentFilter, onChange: (e) => {
                                                setOfferedDepartmentFilter(e.target.value);
                                                setOfferedWorkstationFilter("ALL");
                                            }, className: "px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700", children: [_jsx("option", { value: "ALL", children: "Alle Afdelingen" }), (reportData?.availableDepartments || []).map((dept) => (_jsx("option", { value: dept, children: getDepartmentDisplayLabel(dept) }, dept)))] }), _jsxs("select", { value: offeredWorkstationFilter, onChange: (e) => setOfferedWorkstationFilter(e.target.value), className: "px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700", children: [_jsx("option", { value: "ALL", children: "Alle Werkstations" }), (reportData?.availableWorkstations || []).map((station) => (_jsx("option", { value: station, children: station }, station)))] })] }))] })] }) }), _jsx("div", { className: "p-6", children: _jsx("div", { className: "max-w-7xl mx-auto", children: loading ? (_jsxs("div", { className: "flex flex-col items-center justify-center h-64 gap-4", children: [_jsx(Loader2, { className: "animate-spin text-blue-600", size: 48 }), _jsx("p", { className: "text-sm text-slate-500 font-bold", children: "Rapport wordt gegenereerd..." })] })) : reportData ? (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "grid grid-cols-1 md:grid-cols-4 gap-6", children: [_jsxs("button", { type: "button", onClick: () => selectedReport?.id === "offered_totals" && openKpiPopup("COMPLETED"), className: `p-6 bg-white rounded-2xl border shadow-sm text-left w-full ${selectedReport?.id === "offered_totals" ? "hover:border-blue-300 transition-colors" : "border-slate-200"} ${selectedReport?.id === "offered_totals" && offeredKpiFilter === "COMPLETED" ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200"}`, children: [_jsxs("div", { className: "flex items-center justify-between mb-2", children: [_jsx("span", { className: "text-xs font-bold text-slate-500 uppercase", children: selectedReport?.id === "offered_totals" ? "Gereedgemeld Totaal" : "Totaal" }), reportData.summary.change !== undefined && (_jsxs("div", { className: `px-2 py-1 rounded-lg text-xs font-bold ${reportData.summary.trend === 'up' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`, children: [reportData.summary.trend === 'up' ? '↑' : '↓', " ", Math.abs(reportData.summary.change), "%"] }))] }), _jsx("div", { className: "text-4xl font-black text-slate-800", children: reportData.summary.total.toLocaleString() })] }), selectedReport?.id === "offered_totals" && reportData.summary.offeredTotal !== undefined && (_jsxs("button", { type: "button", onClick: () => openKpiPopup("OFFERED"), className: `p-6 bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-2xl border text-left w-full hover:border-emerald-300 transition-colors ${offeredKpiFilter === "OFFERED" ? "border-emerald-500 ring-2 ring-emerald-100" : "border-emerald-200"}`, children: [_jsx("span", { className: "text-xs font-bold text-emerald-700 uppercase block mb-2", children: "Totaal Aangeboden" }), _jsx("div", { className: "text-4xl font-black text-emerald-900", children: reportData.summary.offeredTotal.toLocaleString() })] })), reportData.summary.ftrPercentage !== undefined && (_jsxs("div", { className: "p-6 bg-gradient-to-br from-green-50 to-green-100 rounded-2xl border border-green-200", children: [_jsx("span", { className: "text-xs font-bold text-green-700 uppercase block mb-2", children: "First Time Right" }), _jsxs("div", { className: "text-4xl font-black text-green-900", children: [reportData.summary.ftrPercentage, "%"] })] })), reportData.summary.completed !== undefined && (_jsxs("div", { className: "p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl border border-blue-200", children: [_jsx("span", { className: "text-xs font-bold text-blue-700 uppercase block mb-2", children: "Voltooid" }), _jsx("div", { className: "text-4xl font-black text-blue-900", children: reportData.summary.completed.toLocaleString() })] })), selectedReport?.id === "offered_totals" && reportData.summary.producedNotOfferedTotal !== undefined && (_jsxs("button", { type: "button", onClick: () => openKpiPopup("PRODUCED_NOT_OFFERED"), className: `p-6 bg-gradient-to-br from-amber-50 to-amber-100 rounded-2xl border text-left w-full hover:border-amber-300 transition-colors ${offeredKpiFilter === "PRODUCED_NOT_OFFERED" ? "border-amber-500 ring-2 ring-amber-100" : "border-amber-200"}`, children: [_jsx("span", { className: "text-xs font-bold text-amber-700 uppercase block mb-2", children: "Geproduceerd, Niet Aangeboden" }), _jsx("div", { className: "text-4xl font-black text-amber-900", children: reportData.summary.producedNotOfferedTotal.toLocaleString() })] })), selectedReport?.id === "offered_totals" && reportData.summary.departmentsWithOutput !== undefined && (_jsxs("div", { className: "p-6 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-2xl border border-indigo-200", children: [_jsx("span", { className: "text-xs font-bold text-indigo-700 uppercase block mb-2", children: "Afdelingen Met Output" }), _jsx("div", { className: "text-4xl font-black text-indigo-900", children: reportData.summary.departmentsWithOutput.toLocaleString() })] })), selectedReport?.id === "offered_totals" && reportData.summary.workstationsWithOutput !== undefined && (_jsxs("div", { className: "p-6 bg-gradient-to-br from-sky-50 to-sky-100 rounded-2xl border border-sky-200", children: [_jsx("span", { className: "text-xs font-bold text-sky-700 uppercase block mb-2", children: "Werkstations Met Output" }), _jsx("div", { className: "text-4xl font-black text-sky-900", children: reportData.summary.workstationsWithOutput.toLocaleString() })] })), reportData.summary.rejected !== undefined && (_jsxs("div", { className: "p-6 bg-gradient-to-br from-red-50 to-red-100 rounded-2xl border border-red-200", children: [_jsx("span", { className: "text-xs font-bold text-red-700 uppercase block mb-2", children: "Afgekeurd" }), _jsx("div", { className: "text-4xl font-black text-red-900", children: reportData.summary.rejected.toLocaleString() })] })), !reportData.summary.ftrPercentage && !reportData.summary.completed && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl border border-blue-200", children: [_jsx("span", { className: "text-xs font-bold text-blue-700 uppercase block mb-2", children: "Gemiddelde per Dag" }), _jsx("div", { className: "text-4xl font-black text-blue-900", children: Math.round(reportData.summary.total / 7).toLocaleString() })] }), _jsxs("div", { className: "p-6 bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl border border-purple-200", children: [_jsx("span", { className: "text-xs font-bold text-purple-700 uppercase block mb-2", children: "Stations Actief" }), _jsx("div", { className: "text-4xl font-black text-purple-900", children: reportData.chartData.length })] })] }))] }), _jsxs("div", { className: "bg-white rounded-2xl border border-slate-200 shadow-sm p-6", children: [_jsx("h3", { className: "text-lg font-black text-slate-800 uppercase tracking-tight mb-6", children: selectedReport?.id === "offered_totals" ? "Gereedgemeld per Afdeling" : "Overzicht per Werkstation" }), selectedReport?.id === "offered_totals" && (_jsxs("p", { className: "text-xs text-slate-500 mb-4", children: ["Actieve KPI-filter: ", offeredKpiFilter === "COMPLETED" ? "Gereedgemeld" : offeredKpiFilter === "OFFERED" ? "Aangeboden" : offeredKpiFilter === "PRODUCED_NOT_OFFERED" ? "Geproduceerd, niet aangeboden" : "Alles"] })), _jsx("div", { className: "space-y-3", children: (selectedReport?.id === "offered_totals" && offeredKpiFilter === "PRODUCED_NOT_OFFERED"
                                            ? (reportData.departmentOverview || []).map((d) => ({ label: getDepartmentDisplayLabel(d.department), value: d.producedNotOffered }))
                                            : (reportData.chartData || []).map((d) => ({ label: getDepartmentDisplayLabel(d.label), value: d.value }))).map((item, index, arr) => {
                                            const maxValue = Math.max(1, ...arr.map((d) => d.value));
                                            const percentage = (item.value / maxValue) * 100;
                                            return (_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: "w-36 shrink-0 text-sm font-black text-slate-700 truncate", title: item.label, children: item.label }), _jsx("div", { className: "flex-1 min-w-0 bg-slate-100 rounded-full h-8 overflow-hidden", children: _jsx("div", { className: "bg-gradient-to-r from-blue-500 to-blue-600 h-full flex items-center justify-end px-3 transition-all duration-500", style: { width: `${percentage}%` }, children: _jsx("span", { className: "text-white text-xs font-bold", children: item.value }) }) })] }, index));
                                        }) })] }), selectedReport?.id === "offered_totals" && Array.isArray(reportData.departmentOverview) && (_jsxs("div", { className: "bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden", children: [_jsx("div", { className: "p-6 border-b border-slate-200", children: _jsx("h3", { className: "text-lg font-black text-slate-800 uppercase tracking-tight", children: "Afdelingsoverzicht Gereedmelding" }) }), _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "bg-slate-50 border-b border-slate-200", children: _jsxs("tr", { children: [_jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Afdeling" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Gereedgemeld (Eindinspectie)" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Geproduceerd, Nog Niet Aangeboden" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-200", children: reportData.departmentOverview
                                                        .filter((dept) => {
                                                        if (offeredKpiFilter === "COMPLETED")
                                                            return dept.completedAtInspection > 0;
                                                        if (offeredKpiFilter === "PRODUCED_NOT_OFFERED")
                                                            return dept.producedNotOffered > 0;
                                                        return true;
                                                    })
                                                        .map((dept) => (_jsxs("tr", { className: "hover:bg-slate-50", children: [_jsx("td", { className: "px-6 py-4 text-sm font-semibold text-slate-800", children: getDepartmentDisplayLabel(dept.department) }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-700", children: dept.completedAtInspection.toLocaleString() }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-700", children: dept.producedNotOffered.toLocaleString() })] }, dept.department))) })] }) })] })), selectedReport?.id === "offered_totals" && Array.isArray(reportData.stationOverview) && (_jsxs("div", { className: "bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden", children: [_jsx("div", { className: "p-6 border-b border-slate-200", children: _jsx("h3", { className: "text-lg font-black text-slate-800 uppercase tracking-tight", children: "Werkstation Overzicht" }) }), _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "bg-slate-50 border-b border-slate-200", children: _jsxs("tr", { children: [_jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Werkstation" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Gereedgemeld" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Geproduceerd, Nog Niet Aangeboden" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-200", children: reportData.stationOverview
                                                        .filter((station) => {
                                                        if (offeredKpiFilter === "COMPLETED")
                                                            return station.completedAtInspection > 0;
                                                        if (offeredKpiFilter === "PRODUCED_NOT_OFFERED")
                                                            return station.producedNotOffered > 0;
                                                        return true;
                                                    })
                                                        .map((station) => (_jsxs("tr", { className: "hover:bg-slate-50", children: [_jsx("td", { className: "px-6 py-4 text-sm font-semibold text-slate-800", children: station.workstation }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-700", children: station.completedAtInspection.toLocaleString() }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-700", children: station.producedNotOffered.toLocaleString() })] }, station.workstation))) })] }) })] })), selectedReport?.id === "offered_totals" && kpiPopup.open && (_jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4", children: _jsxs("div", { className: "bg-white w-full max-w-6xl rounded-2xl border border-slate-200 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col", children: [_jsxs("div", { className: "px-6 py-4 border-b border-slate-200 flex items-center justify-between", children: [_jsxs("div", { children: [_jsxs("h3", { className: "text-xl font-black text-slate-800 uppercase tracking-tight", children: [kpiPopup.type === "COMPLETED" && "KPI Detail: Gereedgemeld", kpiPopup.type === "OFFERED" && "KPI Detail: Aangeboden", kpiPopup.type === "PRODUCED_NOT_OFFERED" && "KPI Detail: Geproduceerd, Niet Aangeboden"] }), _jsx("p", { className: "text-xs text-slate-500 mt-1", children: "Per afdeling en per werkstation binnen de huidige periode/filters." })] }), _jsx("button", { type: "button", onClick: closeKpiPopup, className: "px-3 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-bold hover:bg-slate-200", children: "Sluiten" })] }), _jsxs("div", { className: "p-6 overflow-auto space-y-6", children: [_jsxs("div", { className: "bg-white rounded-2xl border border-slate-200 overflow-hidden", children: [_jsx("div", { className: "px-6 py-4 border-b border-slate-200", children: _jsx("h4", { className: "text-sm font-black text-slate-700 uppercase tracking-wider", children: "Per Afdeling" }) }), _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "bg-slate-50 border-b border-slate-200", children: _jsxs("tr", { children: [_jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Afdeling" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Aantal" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-200", children: (reportData.departmentOverview || [])
                                                                            .map((dept) => ({
                                                                            label: getDepartmentDisplayLabel(dept.department),
                                                                            value: kpiPopup.type === "COMPLETED"
                                                                            ...
                                                                            .sort((a, b) => b.value - a.value)
                                                                            .map((row) => (_jsxs("tr", { className: "hover:bg-slate-50", children: [_jsx("td", { className: "px-6 py-4 text-sm font-semibold text-slate-800", children: row.label }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-700", children: row.value.toLocaleString() })] }, `popup-dept-${row.label}`))) })] }) })] }), _jsxs("div", { className: "bg-white rounded-2xl border border-slate-200 overflow-hidden", children: [_jsx("div", { className: "px-6 py-4 border-b border-slate-200", children: _jsx("h4", { className: "text-sm font-black text-slate-700 uppercase tracking-wider", children: "Per Werkstation" }) }), _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "bg-slate-50 border-b border-slate-200", children: _jsxs("tr", { children: [_jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Werkstation" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Aantal" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-200", children: (reportData.stationOverview || [])
                                                                            .map((station) => ({
                                                                            ...
                                                                            .filter((row) => row.value > 0)
                                                                            .sort((a, b) => b.value - a.value)
                                                                            .map((row) => (_jsxs("tr", { className: "hover:bg-slate-50", children: [_jsx("td", { className: "px-6 py-4 text-sm font-semibold text-slate-800", children: row.label }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-700", children: row.value.toLocaleString() })] }, `popup-station-${row.label}`))) })] }) })] })] })] }) })), selectedReport?.id === "offered_totals" && Array.isArray(reportData.timelineData) && reportData.timelineData.length > 0 && (_jsxs("div", { className: "bg-white rounded-2xl border border-slate-200 shadow-sm p-6", children: [_jsx("h3", { className: "text-lg font-black text-slate-800 uppercase tracking-tight mb-6", children: "Aangeboden Trend (Dag/Week)" }), _jsx("div", { className: "space-y-3", children: reportData.timelineData.map((item, index) => {
                                            const maxValue = Math.max(1, ...reportData.timelineData.map((d) => d.value));
                                            const percentage = (item.value / maxValue) * 100;
                                            return (_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: "w-36 shrink-0 text-sm font-black text-slate-700 truncate", title: item.label, children: item.label }), _jsx("div", { className: "flex-1 min-w-0 bg-slate-100 rounded-full h-8 overflow-hidden", children: _jsx("div", { className: "bg-gradient-to-r from-emerald-500 to-emerald-600 h-full flex items-center justify-end px-3 transition-all duration-500", style: { width: `${percentage}%` }, children: _jsx("span", { className: "text-white text-xs font-bold", children: item.value }) }) })] }, `timeline-${index}`));
                                        }) })] })), _jsxs("div", { className: "bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden", children: [_jsxs("div", { className: "p-6 border-b border-slate-200", children: [_jsx("h3", { className: "text-lg font-black text-slate-800 uppercase tracking-tight", children: "Gedetailleerd Overzicht" }), selectedReport?.id === "product_measurements" && (_jsxs("div", { className: "mt-4 flex flex-wrap items-center gap-2", children: [_jsxs("select", { value: measurementDetailMode, onChange: (e) => {
                                                            const mode = e.target.value;
                                                            setMeasurementDetailMode(mode);
                                                            if (mode !== "browse_week")
                                                                setMeasurementWeekOffset(0);
                                                        }, className: "px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-700", children: [_jsx("option", { value: "current_week", children: "Huidige Week" }), _jsx("option", { value: "browse_week", children: "Terugbladeren per Week" }), _jsx("option", { value: "all", children: "Alles" })] }), measurementDetailMode === "browse_week" && (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", onClick: () => setMeasurementWeekOffset((prev) => prev + 1), className: "px-3 py-2 bg-slate-100 rounded-lg text-xs font-bold text-slate-700 hover:bg-slate-200", children: "\u2190 Oudere Week" }), _jsx("button", { type: "button", onClick: () => setMeasurementWeekOffset((prev) => Math.max(prev - 1, 0)), disabled: measurementWeekOffset === 0, className: "px-3 py-2 bg-slate-100 rounded-lg text-xs font-bold text-slate-700 hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed", children: "Nieuwere Week \u2192" }), _jsx("span", { className: "text-xs text-slate-500 font-semibold", children: measurementWeekOffset === 0 ? "Deze week" : `${measurementWeekOffset} week(en) terug` })] })), _jsx("input", { type: "text", placeholder: "Zoeken op lotnummer...", value: measurementLotNumberSearch, onChange: (e) => setMeasurementLotNumberSearch(e.target.value), className: "px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-700 placeholder-slate-400" })] }))] }), _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "bg-slate-50 border-b border-slate-200", children: _jsxs("tr", { children: [_jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: selectedReport?.id === "lead_time"
                                                                    ? "Doorlooptijd Metric"
                                                                    : selectedReport?.id === "order_completion"
                                                                        ...
                                                                        : selectedReport?.id === "wip_status"
                                                                            ...
                                                                            : selectedReport?.id === "production_output"
                                                                                ? "Output Segment"
                                                                                : "Product" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: selectedReport?.id === "product_measurements"
                                                                    ? "Metingen"
                                                                    : selectedReport?.id === "lead_time"
                                                                        ? "Waarde"
                                                                        : selectedReport?.id === "order_completion"
                                                                            ? "Voortgang"
                                                                            : "Aantal" }), selectedReport?.id === "product_measurements" && (_jsxs(_Fragment, { children: [_jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "TG" }), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Brix" })] })), _jsx("th", { className: "px-6 py-3 text-left text-xs font-black text-slate-600 uppercase tracking-wider", children: "Status" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-200", children: reportData.details
                                                        .filter((detail) => {
                                                        ...
                                                        .map((detail) => (_jsxs("tr", { className: "hover:bg-slate-50", children: [_jsx("td", { className: "px-6 py-4 text-sm font-medium text-slate-800", children: detail.name }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-600", children: selectedReport?.id === "product_measurements" ? (_jsxs("div", { className: "space-y-1", children: [_jsx("div", { className: "font-semibold text-slate-700", children: detail.measurementSummary || "Geen meetwaarden" }), _jsxs("div", { className: "text-xs text-slate-400", children: [detail.count, " velden"] })] })) : (detail.count) }), selectedReport?.id === "product_measurements" && (_jsxs(_Fragment, { children: [_jsx("td", { className: "px-6 py-4 text-sm text-slate-700 font-semibold", children: detail.tgValue || "-" }), _jsx("td", { className: "px-6 py-4 text-sm text-slate-700 font-semibold", children: detail.brixValue || "-" })] })), _jsx("td", { className: "px-6 py-4", children: _jsx("span", { className: `px-2 py-1 rounded-lg text-xs font-bold ${detail.status === "completed"
                                                                            ...
                                                                            : detail.status === "temp_reject"
                                                                                ...
                                                                                : detail.status === "active"
                                                                                    ...
                                                                                    : detail.status === "idle"
                                                                                        ...
                                                                                        : "bg-yellow-100 text-yellow-700"}`, children: detail.status === "completed"
                                                                        ? "Voltooid"
                                                                        : detail.status === "rejected"
                                                                            ? "Afgekeurd"
                                                                            : detail.status === "temp_reject"
                                                                                ...
                                                                                : detail.status === "active"
                                                                                    ...
                                                                                    : detail.status === "idle"
                                                                                        ? "Niet Actief"
                                                                                        : "In behandeling" }) })] }, detail.id))) })] }) })] })] })) : (_jsxs("div", { className: "flex flex-col items-center justify-center h-64 gap-4", children: [_jsx(FileText, { className: "text-slate-300", size: 64 }), _jsx("p", { className: "text-slate-500 text-sm font-bold", children: "Geen data beschikbaar" }), _jsx("button", { onClick: generateReportData, className: "px-6 py-2 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 transition-colors", children: "Genereer Rapport" })] })) }) })] }));
};
