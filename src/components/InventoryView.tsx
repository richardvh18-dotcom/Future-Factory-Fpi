    ...
    <div className="max-w-7xl mx-auto p-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {items
          ...
          .map((item) => (
            <div key={item.id} className="bg-white border border-slate-200 p-5 shadow-sm hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <div className="bg-slate-100 p-2 text-slate-600">
                  {item.category === "Tool" ? <Wrench size={20} /> : <Box size={20} />}
                </div>
                <span className={`text-[10px] font-black uppercase px-2 py-1 ${getStatusColor(
              ...
              <p className="text-[10px] text-slate-400 font-mono mb-4">{t('inventory.id', 'ID')}: {item.id}</p>
              ...
              <div className="space-y-3 pt-4 border-t border-slate-50">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-600">
                  ...
                  <span>{t('inventory.assigned_to', 'Toegewezen aan')}: <strong>{item.assignedTo || t('inventory.nobody', 'Niemand')}</strong></span>
                </div>
              </div>
