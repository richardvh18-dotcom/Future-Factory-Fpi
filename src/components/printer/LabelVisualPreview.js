...
const LabelVisualPreview = ({ label, data, zoom = 1, className = "", printerDpi = 203 }) => {
    ...
    if (!label)
        return _jsx("div", { className: `w-48 h-32 bg-slate-200 flex items-center justify-center text-xs text-slate-400 italic ${className}`, children: "Geen template" });
    return (_jsx("div", { className: `bg-white relative overflow-hidden ${className}`, style: {
            width: `${label.width * pixelsPerMm * zoom}px`,
            height: `${label.height * pixelsPerMm * zoom}px`,
        }, children: label.elements?.map((el, index) => {
            ...
            if (el.type === "qr" || el.type === "barcode")
                return (_jsx("div", { style: {
                    ...
                    }, children: el.type === 'barcode' ? (_jsx("img", { src: getBarcodeUrl(displayContent), alt: "code", style: { width: "80%", height: "80%", objectFit: "contain" } })) : (_jsx(InternalQrImage, { value: displayContent, size: 160, alt: "code", className: "w-[80%] h-[80%] object-contain" })) }, index));
            return null;
        }) }));
};
