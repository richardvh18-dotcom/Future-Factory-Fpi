...
const getOrderLabelOrder = (item = {}) => item.orderId ||
    item.orderNumber ||
    item.Order ||
    item.Productieorder ||
    item.order ||
    ...
    item.id ||
    "ONBEKEND";
const getOrderLabelItemCode = (item = {}) => item.itemCode ||
    item.productCode ||
    item.articleCode ||
    item.productId ||
    item.Item ||
    ...
    item.item ||
    "";
const getOrderLabelDescription = (item = {}) => item.itemDescription ||
    item.description ||
    item.Description ||
    item.Omschrijving ||
    item.item ||
...
const TempLabelModal = ({ onClose, onPrint, labelTemplates = [], labelRules = [], printerDpi = 300 }) => {
    ...
    useEffect(() => {
        ...
        const loadInitialList = async () => {
            ...
            try {
                ...
                dedup.sort((a, b) => String(getOrderLabelOrder(a)).localeCompare(String(getOrderLabelOrder(b)), undefined, { numeric: true }));
    ...
    return (_jsx("div", { className: "fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in", children: _jsxs("div", { className: "bg-white w-full max-w-4xl rounded-[40px] shadow-2xl overflow-hidden relative border border-slate-100", children: [_jsx("div", { className: "absolute top-0 right-0 p-8 opacity-5 pointer-events-none", children: _jsx(Tag, { size: 180, className: "text-slate-900 -rotate-12" }) }), _jsxs("div", { className: "p-8 md:p-10 relative z-10 flex flex-col h-full max-h-[90vh]", children: [_jsxs("div", { className: "flex justify-between items-start mb-8", children: [_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: "p-4 bg-amber-50 text-amber-600 rounded-2xl shadow-sm border border-amber-100/50", children: _jsx(Tag, { size: 28, strokeWidth: 2.5 }) }), _jsxs("div", { children: [_jsxs("h3", { className: "text-2xl font-black text-slate-900 uppercase italic tracking-tighter leading-none mb-1", children: ["Order ", _jsx("span", { className: "text-amber-600", children: "Labels" })] }), _jsx("p", { className: "text-[10px] font-bold text-slate-400 uppercase tracking-widest", children: "Legacy / Nood-etiketten zoeken" })] })] }), _jsx("button", { onClick: onClose, className: "p-2 bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-slate-600 rounded-full transition-colors", children: _jsx(X, { size: 20 }) })] }), _jsxs("div", { className: "flex flex-col sm:flex-row gap-3 mb-6 shrink-0", children: [_jsxs("div", { className: "relative flex-1 group", children: [_jsx(Search, { className: "absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-amber-500 transition-colors", size: 18 }), _jsx("input", { type: "text", placeholder: t('printer.searchOrderPlaceholder', 'ZOEK OP ORDER OF PRODUCT'), className: "w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold uppercase outline-none focus:bg-white focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all text-sm text-slate-900 placeholder:text-slate-400", value: orderStr, onChange: (e) => setOrderStr(e.target.value), onKeyDown: (e) => e.key === 'Enter' && handleOrderLabelSearch() })] }), _jsx("button", { onClick: handleOrderLabelSearch, disabled: loading, className: "px-8 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-amber-500 transition-all flex items-center justify-center gap-2 shadow-xl active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed", children: loading ? _jsx(Loader2, { className: "animate-spin", size: 18 }) : t("common.search") })] }), _jsxs("div", { className: "flex-1 overflow-y-auto custom-scrollbar -mx-2 px-2 pb-2", children: [displayItems.length > 0 && (_jsx("div", { className: "space-y-3", children: displayItems.map((item, idx) => (_jsx(TempLabelItem, { item: item, labelTemplates: labelTemplates, labelRules: labelRules, onPrint: onPrint, isExpanded: expandedItemId === (item.id || idx), onToggle: () => setExpandedItemId(expandedItemId === (item.id || idx) ? null : (item.id || idx)), printerDpi: printerDpi }, idx))) })), loadingInitialList && !orderStr.trim() && (_jsxs("div", { className: "py-12 border-2 border-dashed border-slate-200 rounded-[30px] flex flex-col items-center justify-center text-center bg-slate-50/50", children: [_jsx(Loader2, { className: "animate-spin text-slate-400 mb-3", size: 24 }), _jsx("p", { className: "text-xs text-slate-400 font-medium", children: t("common.loadingList") })] })), results.length === 0 && orderStr.trim() && !loading && (_jsxs("div", { className: "py-12 border-2 border-dashed border-slate-200 rounded-[30px] flex flex-col items-center justify-center text-center bg-slate-50/50", children: [_jsx("div", { className: "p-4 bg-slate-100 text-slate-400 rounded-full mb-3", children: _jsx(Search, { size: 24 }) }), _jsx("p", { className: "text-sm font-black text-slate-600 uppercase tracking-widest", children: t("common.nothingFound") }), _jsx("p", { className: "text-xs text-slate-400 font-medium mt-1", children: t("common.noOrderOrProductFoundFor", { query: orderStr }) })] }))] })] })] }) }));
};
...
^XZ`;
    ...
    return (_jsx("div", { className: "p-4 md:p-8 bg-slate-50 min-h-screen", children: _jsxs("div", { className: "max-w-4xl mx-auto", children: [_jsxs("div", { className: "flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Printer, { className: "text-slate-800", size: 32 }), _jsx("h1", { className: "text-3xl font-bold text-slate-800", children: "Centraal Printstation" })] }), _jsxs("div", { className: "flex items-center gap-3", children: [('usb' in navigator) && (_jsxs("button", { onClick: handleConnectUsb, className: `flex items-center gap-2 px-4 py-2 rounded-xl font-bold uppercase text-xs tracking-wider transition-all shadow-sm border-2 ${usbDevice ? 'bg-green-50 text-green-700 border-green-200' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300'}`, children: [_jsx(Usb, { size: 16, className: usbDevice ? "text-green-500" : "" }), _jsx("span", { className: "hidden sm:inline", children: usbDevice ? `USB: ${usbDevice.productName}` : 'Koppel USB Printer' })] })), _jsxs("button", { onClick: () => setShowLotModal(true), className: "bg-blue-600 text-white px-4 py-2 rounded-xl font-bold uppercase text-xs tracking-wider flex items-center gap-2 hover:bg-blue-700 transition-all shadow-sm w-fit", children: [_jsx(Printer, { size: 16 }), " Lotnummers"] }), _jsxs("button", { onClick: () => setShowTempModal(true), className: "bg-amber-500 text-white px-4 py-2 rounded-xl font-bold uppercase text-xs tracking-wider flex items-center gap-2 hover:bg-amber-600 transition-all shadow-sm w-fit", children: [_jsx(Tag, { size: 16 }), " Order Labels"] })] })] }), _jsx("p", { className: "text-slate-600 mb-8", children: "Scan of typ een lotnummer om een label te (her)printen. De printopdracht wordt naar de centrale printer bij BH18 gestuurd." }), _jsxs("form", { onSubmit: handleLotNumberSearch, className: "flex gap-2 mb-8", children: [_jsxs("div", { className: "relative flex-grow", children: [_jsx(Search, { className: "absolute left-3 top-1/2 -translate-y-1/2 text-slate-400", size: 20 }), _jsx("input", { type: "text", value: lotNumber, onChange: (e) => setLotNumber(e.target.value.toUpperCase()), placeholder: "Scan of typ lotnummer...", className: "w-full p-3 pl-10 border-2 border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" })] }), _jsxs("button", { type: "submit", disabled: isLoading || !lotNumber, className: "bg-slate-800 text-white px-6 py-3 rounded-lg font-semibold hover:bg-slate-700 disabled:bg-slate-400 flex items-center gap-2", children: [isLoading ? _jsx(Loader2, { className: "animate-spin" }) : _jsx(Search, { size: 20 }), _jsx("span", { children: "Zoek" })] })] }), error && _jsx("div", { className: "text-red-600 bg-red-100 p-4 rounded-lg mb-8", children: error }), productData && (_jsxs("div", { className: "bg-white p-6 rounded-lg shadow-md animate-in fade-in", children: [_jsxs("h2", { className: "text-2xl font-bold mb-4", children: ["Product Gevonden: ", productData.lotNumber] }), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-6", children: [_jsxs("div", { children: [_jsxs("p", { children: [_jsx("strong", { children: "Order:" }), " ", productData.orderId] }), _jsxs("p", { children: [_jsx("strong", { children: "Artikel:" }), " ", productData.itemCode] }), _jsxs("p", { children: [_jsx("strong", { children: "Omschrijving:" }), " ", productData.item] }), _jsxs("div", { className: "mt-4", children: [_jsx("label", { htmlFor: "label-select", className: "block text-sm font-medium text-slate-700 mb-1", children: "Kies Label Template" }), _jsx("select", { id: "label-select", value: selectedLabelId, onChange: (e) => setSelectedLabelId(e.target.value), className: "w-full p-2 border border-slate-300 rounded-md", children: availableLabels.map(l => _jsxs("option", { value: l.id, children: [l.name, " (", l.width, "x", l.height, "mm)"] }, l.id)) })] }), _jsxs("button", { onClick: handlePrint, disabled: isLoading, className: "mt-6 w-full bg-blue-600 text-white px-6 py-4 rounded-lg font-bold text-lg hover:bg-blue-500 disabled:bg-blue-300 flex items-center justify-center gap-3", children: [isLoading ? _jsx(Loader2, { className: "animate-spin" }) : _jsx(Send, { size: 24 }), _jsx("span", { children: "Stuur naar Printer" })] })] }), _jsxs("div", { className: "bg-slate-800 p-4 rounded-lg", children: [_jsx("h3", { className: "text-white font-bold mb-2", children: "Label Preview" }), selectedLabel ? _jsx(AutoScaledLabelPreview, { label: selectedLabel, data: previewData, className: "mx-auto", printerDpi: printerDpi }) : _jsx("p", { className: "text-slate-400", children: "Selecteer een label" })] })] })] })), showTempModal && (_jsx(TempLabelModal, { onClose: () => setShowTempModal(false), onPrint: handleTempLegacyPrint, labelTemplates: labelTemplates, labelRules: labelRules, printerDpi: printerDpi })), showLotModal && (_jsx(LotPrintModal, { onClose: () => setShowLotModal(false), departmentGroups: departmentGroups, onPrintBatch: handleDirectLotPrintBatch, printer: activeQueuePrinter }))] }) }));
};
