 ...
 *
 * @param timeoutMinutes - Aantal minuten inactiviteit voor uitloggen (default: 30)
 * @param warningMinutes - Aantal minuten voor waarschuwing voordat uitloggen (default: 5)
 * @param enabled - Of auto-logout actief is (default: true)
 */
export const useAutoLogout = (timeoutMinutes = 30, warningMinutes = 5, enabled = true) => {
    ...
    useEffect(() => {
        ...
        resetTimer();
        return () => {
            events.forEach((event) => document.removeEventListener(event, activityHandler, true));
            clearAllTimers();
        };
    }, [enabled, showWarning]);
    return { showWarning, remainingTime, dismissWarning, handleActivity };
};
