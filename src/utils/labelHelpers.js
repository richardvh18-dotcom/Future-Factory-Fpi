...
const parseConnections = (text) => {
    ...
    const match = text.match(/\b(TB|CB|FL|AM|AB|CS|CF|FB)-?(TB|CB|FL|AM|AB|CS|CF|FB)(-?(TB|CB|FL|AM|AB|CS|CF|FB))?\b/i);
    if (match) {
...
const parseProductType = (text) => {
    ...
    if (type === "ELBOW") {
        const degreeMatch = text.match(/\/(\d+)/) || text.match(/(\d+)\s*(?:DEG|GR|°)/i);
        if (degreeMatch) {
            type += ` ${degreeMatch[1]}°`;
        }
        else {
...
const parseDimensions = (text, productType, data = {}) => {
    ...
    const needsDualId = productType.includes("REDUCER") ||
    ...
    if ((!val || val <= 25 || val === 45 || val === 90) && (data.dn || data.diameter)) {
        val = parseInt(data.dn) || parseInt(data.diameter);
    }
...
export const processLabelData = (data) => {
    ...
    // 5. Radius logic
    let radiusText = "";
    if (desc.toUpperCase().includes("R1.0") ||
...
export const applyLabelLogic = (data, rules) => {
    ...
    const activeRule = rules.find((r) => String(r.productCode || "").toUpperCase() === productCode);
            ...
            if (variable.mappings) {
                ...
                const getValue = (field) => {
                    ...
                    if (data[field] !== undefined)
                        return data[field];
                    // 2. Mapping aliases
                    if (field === 'diameter' && data.dn !== undefined)
                        return data.dn;
                    if (field === 'pressure' && data.pn !== undefined)
                        return data.pn;
                    if (field === 'extraCode')
                        return data.extraCode || data.code;
                    // 3. Specs object
                    const specs = data.specs;
                    if (specs && specs[field] !== undefined)
                        return specs[field];
                ...
                const match = variable.mappings.find((m) => {
                    ...
                    if (!isNaN(numVal)) {
                        if (condition.startsWith(">="))
                            return numVal >= parseFloat(condition.substring(2));
                        if (condition.startsWith("<="))
                            return numVal <= parseFloat(condition.substring(2));
                        if (condition.startsWith(">"))
                            return numVal > parseFloat(condition.substring(1));
                        if (condition.startsWith("<"))
                            return numVal < parseFloat(condition.substring(1));
                    }
                    if (condition.startsWith("!="))
                        return val.toUpperCase() !== condition.substring(2).trim().toUpperCase();
                    if (condition.startsWith("=="))
                        return val.toUpperCase() === condition.substring(2).trim().toUpperCase();
...
export const resolveLabelContent = (contentOrElement, data) => {
    ...
    if (isElement) {
        const element = contentOrElement;
        return {
            ...element,
            content: content,
        };
...
export const filterLabelsByProduct = (labels, product, options = {}) => {
    ...
    const sourceLabels = excludeTempOrderLabels
        ? labels.filter((label) => {
            ...
            return (!tags.includes("TIJDELIJK") &&
                !tags.includes("TEMP") &&
    ...
    if (desc.includes("WAVISTRONG") || type.includes("WAVISTRONG"))
        productTags.add("WAVISTRONG");
    if (desc.includes("FIBERMAR") || type.includes("FIBERMAR"))
        productTags.add("FIBERMAR");
    if (desc.includes("EST"))
        productTags.add("EST");
    if (desc.includes("CST"))
        productTags.add("CST");
    if (desc.includes("EMT"))
        productTags.add("EMT");
    if (desc.includes("EWT"))
        productTags.add("EWT");
    // 2. Filter logica
...
export const getBarcodeUrl = (data) => `https://bwipjs-api.metafloor.com/?bcid=code128&text=${encodeURIComponent(String(data ?? "leeg"))}&scale=3&height=10&incltext&guardwhitespace`;
