...
export const calculateOrderPriorityScore = (order) => {
    let score = 0;
    // 1. Order Type weging
    const typeStr = String(order.type || '').toLowerCase();
    if (typeStr.includes('klant'))
        score += 50;
    else if (typeStr.includes('project'))
        score += 30;
    else if (typeStr.includes('voorraad'))
        score += 5;
    else
        score += 15; // Standaard / Onbekend
    // 2. Handmatige (bestaande) prioriteit respecteren
    if (order.priority === 'immediate')
        score += 150;
...
export const generateSmartSuggestions = (orders) => {
    // Filter alleen orders die nog niet gereed of geannuleerd zijn
    const activeOrders = orders.filter(o => !['completed', 'gereed', 'rejected', 'cancelled', 'deleted'].includes(o.status?.toLowerCase()));
