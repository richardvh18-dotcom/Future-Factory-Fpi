...
const toMillis = (value) => {
    ...
    const ms = value instanceof Date
        ? value.getTime()
        : new Date(typeof value === "string" || typeof value === "number"
            ? value
...
export const subscribeTrackedProducts = ({ db, onData, onError, statusExclusions = [], maxItems = null, departments = DEFAULT_TRACKING_DEPARTMENTS, machines = DEFAULT_TRACKING_MACHINES, }) => {
    let rootDocs = [];
    ...
    const rootUnsub = onSnapshot(collection(db, getPathString(PATHS.TRACKING)), (snap) => {
    ...
    const scopedUnsubs = scopedTargets.map(({ department, machine, key }) => onSnapshot(collection(db, getPathString([...PATHS.TRACKING, department, "machines", machine, "items"])), (snap) => {
        scopedBuckets.set(key, snap.docs.map((docSnap) => ({
            ...docSnap.data(),
            id: docSnap.id,
            __docPath: docSnap.ref.path,
            sourcePath: docSnap.ref.path,
...
export const trackedLotExistsActive = async ({ db, lotNumber, excludeDocId = null, }) => {
    ...
    const scopedSnaps = await Promise.all(scopedTargets.map(({ department, machine }) => getDocs(query(collection(db, getPathString([...PATHS.TRACKING, department, "machines", machine, "items"])), where("lotNumber", "==", normalizedLot), limit(1)))));
