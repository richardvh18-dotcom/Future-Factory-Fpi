...
export const getMaterialInfo = (itemCode) => {
    if (!itemCode)
        return { material: "Unknown", diameter: null };
...
export const getStepForStation = (stationName) => {
    ...
    if (name.includes("BM01"))
        return { status: FLOW_STATUS.TE_KEUREN, currentStep: FLOW_STEPS.EINDINSPECTIE };
    if (name.includes("NABEWERK") || name.includes("MAZAK"))
        return { status: FLOW_STATUS.TE_NABEWERKEN, currentStep: FLOW_STEPS.NABEWERKING };
    if (name === "LOSSEN")
        return { status: FLOW_STATUS.IN_PROGRESS, currentStep: FLOW_STEPS.LOSSEN };
