...
type TimeInput = Date | string | number | { toDate: () => Date } | null | undefined;

type BatchEfficiencyItem = {
...
const toDateValue = (input: TimeInput): Date => {
...
export const calculateDuration = (startTime: TimeInput, endTime: TimeInput = new Date()) => {
 ...
 */
export const calculateAvailability = (operatingTimeMinutes: number, plannedBusyTimeMinutes: number) => {
 ...
 */
export const calculatePerformance = (actualOutput: number, targetOutput: number) => {
 ...
 * @returns {number} Percentage (0-100)
 */
export const calculateQuality = (goodCount: number, totalCount: number) => {
 ...
 */
export const calculateOEE = (availabilityPct: number, performancePct: number, qualityPct: number) => {
 ...
 */
export const calculateEfficiency = (actualMinutes: number, targetMinutes: number) => {
...
export const isBehindSchedule = (startTime: TimeInput, targetMinutes: number) => {
 ...
 */
export const calculateTimeDeviation = (startTime: TimeInput, targetMinutes: number) => {
  const elapsed = calculateDuration(startTime);
  return elapsed - targetMinutes;
};
 ...
 */
export const calculateBatchEfficiency = (products: BatchEfficiencyItem[] | null | undefined) => {
 ...
 */
export const getEfficiencyLabel = (efficiency: number) => {
