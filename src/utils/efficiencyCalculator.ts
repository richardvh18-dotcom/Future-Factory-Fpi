...
export const calculateDuration = (startTime, endTime = new Date()) => {
  ...
  const start = (startTime as any).toDate ? (startTime as any).toDate() : new Date(startTime);
  const end = (endTime as any).toDate ? (endTime as any).toDate() : new Date(endTime as any);
