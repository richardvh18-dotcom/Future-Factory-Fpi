...
import { upsertConversionRecord, deleteConversionRecord, upsertConversionBatch, } from "../services/planningSecurityService";
/**
...
export const lookupProductByManufacturedId = async (unusedAppId, inputCode) => {
    ...
    try {
        ...
        const docIdCandidates = Array.from(new Set([rawCode, normalizedCode, compactNormalizedCode].filter(Boolean)));
...
export const findConversionCandidate = async (unusedAppId, specs) => {
    try {
        ...
        const q = query(recordsRef, where("type", "==", String(specs.type || "")), where("dn", "==", parseInt(String(specs.dn || "0"), 10)), where("pn", "==", parseFloat(String(specs.pn || "0"))));
        ...
        const bestMatch = results.find((r) => String(r.serie || "") === String(specs.serie || "") || r.description?.includes(String(specs.label || "")));
...
export const fetchConversions = async (unusedAppId, lastDoc = null, pageSize = 50, searchTerm = "") => {
    ...
    if (lastDoc) {
        q = query(conversionsRef, orderBy("manufacturedId"), startAfter(lastDoc), limit(pageSize));
    }
    ...
    if (searchTerm) {
        ...
        filteredData = data.filter((item) => item.manufacturedId.toLowerCase().includes(s) ||
...
export const uploadConversionBatch = async (items, unusedAppId, onProgress) => {
    ...
    if (onProgress)
        onProgress(100);
    await logActivity(getActorId(), "CONVERSION_BATCH_UPLOAD", `Conversie batch-upload voltooid: ${total} records verwerkt`);
};
/**
...
export const uploadNewItemsOnly = async (items, unusedAppId, onProgress) => {
    ...
    await logActivity(getActorId(), "CONVERSION_NEWITEMS_UPLOAD", `Nieuwe conversie-items upload: toegevoegd ${added}, overgeslagen ${skipped}`);
