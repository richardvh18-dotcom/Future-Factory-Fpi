...
const closeMatchingAuthorizedDevices = async ({ printer = {}, excludeDevice } = {}) => {
    ...
    const matching = authorizedDevices.filter((device) => {
        ...
        if (filters.length === 0)
            return true;
        return filters.some((f) => device.vendorId === f.vendorId && (f.productId ? device.productId === f.productId : true));
    });
...
const normalizeUsbError = (err) => {
    ...
    if (name === "SecurityError" || /access denied|permission|toegang|not allowed/i.test(combined)) {
        return new Error("USB toegang geweigerd. Sluit andere tabbladen/apps die de printer gebruiken, koppel USB opnieuw en geef browsertoegang opnieuw.");
    }
    return err;
};
...
export const findAuthorizedUsbDevice = async (printer = {}) => {
    ...
    return (authorizedDevices.find((d) => filters.some((f) => d.vendorId === f.vendorId && (f.productId ? d.productId === f.productId : true))) || null);
};
