...
export const evaluateOrderDelay = async (conditions) => {
    ...
    const delayedOrders = planning.filter(p => {
        if (!p.plannedDate || p.status === "shipped" || p.status === "completed")
            return false;
...
export const evaluateInspectionOverdue = async (conditions) => {
    ...
    const overdueProducts = products.filter(p => {
        ...
        if (station && p.currentStation !== station)
            return false;
...
export const evaluateStandardDeviation = async (conditions) => {
    const { minSamples = 5, minDeviation = 5 // percentage
     } = conditions;
    ...
    for (const standard of standards) {
        ...
        const validProducts = products.filter(p => p.timestamps?.station_start &&
...
export const checkDebounce = async (ruleId, debounceMinutes = 60) => {
    ...
    const executionsSnap = await getDocs(query(collection(db, ...PATHS.AUTOMATION_EXECUTIONS), where("ruleId", "==", ruleId)));
