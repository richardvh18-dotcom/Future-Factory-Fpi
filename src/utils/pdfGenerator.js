...
const imageToDataUri = async (url) => {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(i18n.t("pdf.netwerk_error", "Netwerk response was niet ok"));
        ...
        console.error(i18n.t("pdf.image_download_error", "PDF Generator Error: Afbeelding kon niet worden gedownload."), error);
        return null;
    }
};
/**
...
export const generateProductPDF = async (product, role = "operator") => {
    ...
    // --- 1. TITEL OPBOUW ---
    const typePart = product.type?.replace("_Socket", "").replace("_SOCKET", "") || "";
    const anglePart = product.angle && product.angle !== "-" ? `${product.angle}°` : "";
    const radiusPart = product.radius && product.radius !== "-" ? product.radius : "";
    ...
    const fullTitle = `${typePart} ${anglePart} ${radiusPart} ${connPart} ${idPart} ${pnPart}`
        .replace(/\s+/g, " ")
        .trim();
    // --- 2. HEADER ONTWERP ---
    ...
    if (product.imageUrl) {
        ...
        if (imgData) {
            ...
            doc.addImage(imgData, "PNG", xPos, currentY, imgWidth, imgHeight, undefined, "FAST");
