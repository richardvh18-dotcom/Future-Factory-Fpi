    ...
    try {
        ...
        for (let i = 0; i < total; i++) {
            ...
            const planningCode = [
                ...
                .find(Boolean);
            if (planningCode) {
                const conversion = (await lookupProductByManufacturedId(appId, planningCode));
                ...
                if (resolvedCode) {
                    ...
                    if (prodSnap.exists()) {
                        ...
                        const qProd = query(collection(db, getPathString(PATHS.PRODUCTS)), where("articleCode", "==", newCode));
                        const qSnap = await getDocs(qProd);
                        if (!qSnap.empty)
                            productDoc = qSnap.docs[0].data();
                    }
                    if (productDoc &&
