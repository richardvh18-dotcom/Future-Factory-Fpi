...
const getResolvedTextMaxLines = (el, requestedHeight, rot = "N") => {
    ...
    const lineBudgetMm = (rot === 'R' || rot === 'B')
        ...
        : (Number(el.height) || 0);
    if (!lineBudgetMm || !requestedHeight)
        return 1;
...
export const generatePrintData = (template, data = {}, printerDpi = 203, resolveFn = null, t = null) => {
    ...
    const { width = 90, height = 40, elements = [], darkness = 15, printSpeed = 3 } = templateInput;
    const { isLastOfBatch = true, columnIndex = 0, useDynamicLength = false // Kan true zijn als template dat toestaat
    ...
    elements.forEach((el) => {
        ...
        const normalizedRotation = rotationValue === 90 || rotationValue === 180 || rotationValue === 270 ? rotationValue : 0;
        ...
        if (el.type === 'text') {
            ...
            if (widthDots && !isVerticalRotation) {
                ...
                const alignKey = el.align === 'center' || el.align === 'right' || el.align === 'justify'
                    ? el.align
                    : 'left';
 ...
 */
export const generateLotBatchZPL = ({ lots = [], orderNumber = "", printerDpi = 203, darkness = 15, labelWidthMm = 90, labelHeightMm = 13, qrSizeMm = 9, qrXmm = 2, qrYmm = 2.5, qrCellWidth = null, textYmm = 2.5, textHeightMm = 6.5, rightMarginMm = 2, gapAfterQrMm = 2, lotTextShiftRightMm = 0, separatorXmm = 2, separatorYmm = 12.4, separatorWidthMm = 86 } = {}) => {
    ...
    const textX = Math.max(textAreaStartDots, textAreaStartDots + Math.round((textAreaWidthDots - estimatedTextWidthDots) / 2) + lotTextShiftRightDots);
    let zplBatch = "";
...
export const logPrintAction = async (user, labelName, printerIp, data) => {
    try {
        ...
        await logActivity(user?.uid || "system", "PRINT_LABEL", `Label geprint: ${labelName || "Unknown Label"} voor lot ${data?.lotNumber || "N/A"} via ${printerIp || "Local/PDF"}`);
    }
    catch (e) {
...
export const checkPrinterStatus = async (ip) => {
    if (!ip)
        return { online: false, message: "No IP" };
    try {
