...
const isDriverId = (value) => typeof value === 'string' && value in PRINTER_DRIVERS;
/**
 ...
 */
export const getDriver = (printerProfile) => {
    if (!printerProfile)
    ...
    if (isDriverId(stored))
        return PRINTER_DRIVERS[stored];
...
export const getPrinterRollSettings = (printer = {}) => {
 ...
 */
export const applyCalibration = (zplContent, printer = {}, driver = null) => {
    ...
    if (rollSettings.mediaTrackingCommand)
        insertionParts.push(rollSettings.mediaTrackingCommand);
    if (xPart)
        insertionParts.push(xPart);
    if (yPart)
        insertionParts.push(yPart);
