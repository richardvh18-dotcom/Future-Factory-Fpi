...
export const archiveOrder = async (order, reason) => {
    ...
    try {
        ...
        const archiveYear = resData?.archiveYear || new Date().getFullYear();
        console.log(i18n.t("archive.success", { order: order.orderId || order.id, year: archiveYear, defaultValue: `Order ${order.orderId || order.id} succesvol verplaatst naar archief (${archiveYear})` }));
        return true;
    }
    catch (error) {
