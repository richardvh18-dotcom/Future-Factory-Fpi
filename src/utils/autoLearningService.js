...
export const analyzeAndUpdateStandards = async (options = {}) => {
    const { minSamples = 5, maxDeviation = 50, learningRate = 0.3, dryRun = false } = options;
    const results = {
        analyzed: 0,
        updated: 0,
        skipped: 0,
    ...
    try {
        ...
        for (const standard of standards) {
            ...
            try {
                ...
                try {
                    const trackedQuery = query(collection(db, ...PATHS.TRACKING), where("item", "==", standard.itemCode), where("originMachine", "==", standard.machine), where("status", "==", "completed"));
                ...
                if (completedProducts.length === 0) {
                    try {
                        const scopedQuery = query(collectionGroup(db, 'items'), where("item", "==", standard.itemCode), where("originMachine", "==", standard.machine), where("status", "==", "completed"));
                ...
                const validProducts = completedProducts.filter(p => p.timestamps?.station_start &&
                    (p.timestamps?.completed || p.timestamps?.finished));
                if (validProducts.length < minSamples) {
                    results.skipped++;
                    console.log(i18n.t("autolearning.skipped_samples", { item: standard.itemCode, machine: standard.machine, count: validProducts.length, min: minSamples, defaultValue: `[Auto-Learning] Skipped ${standard.itemCode}/${standard.machine}: only ${validProducts.length} samples (min: ${minSamples})` }));
                    continue;
                }
                // Bereken werkelijke tijden
                const actualTimes = validProducts.map(p => {
                    const duration = calculateDuration(p.timestamps.station_start, p.timestamps.completed || p.timestamps.finished);
                    return duration;
                }).filter(t => t > 0);
                ...
                if (Math.abs(deviation) < 5) {
                    ...
                    console.log(i18n.t("autolearning.deviation_acceptable", { item: standard.itemCode, machine: standard.machine, deviation: deviation.toFixed(1), defaultValue: `[Auto-Learning] ${standard.itemCode}/${standard.machine}: deviation ${deviation.toFixed(1)}% is acceptable` }));
                    continue;
                }
                ...
                if (Math.abs(deviation) > maxDeviation) {
                    ...
                    console.warn(i18n.t("autolearning.deviation_extreme", { item: standard.itemCode, machine: standard.machine, deviation: deviation.toFixed(1), defaultValue: `[Auto-Learning] ${standard.itemCode}/${standard.machine}: deviation ${deviation.toFixed(1)}% too extreme, skipping` }));
                    continue;
                }
                ...
                if (!dryRun) {
                    ...
                    results.updated++;
                    console.log(i18n.t("autolearning.updated", { item: standard.itemCode, machine: standard.machine, old: currentStandard, new: roundedNew, samples: actualTimes.length, deviation: deviation.toFixed(1), defaultValue: `[Auto-Learning] Updated ${standard.itemCode}/${standard.machine}: ${currentStandard}m → ${roundedNew}m (${actualTimes.length} samples, ${deviation.toFixed(1)}% deviation)` }));
                }
                else {
                    console.log(i18n.t("autolearning.dry_run", { item: standard.itemCode, machine: standard.machine, old: currentStandard, new: roundedNew, defaultValue: `[Auto-Learning] [DRY RUN] Would update ${standard.itemCode}/${standard.machine}: ${currentStandard}m → ${roundedNew}m` }));
                }
            }
            catch (error) {
