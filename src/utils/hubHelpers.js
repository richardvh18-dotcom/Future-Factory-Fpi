...
const isFirestoreDateLike = (value) => typeof value === "object" && value !== null && "toDate" in value;
// --- CONFIGURATIE ---
export const getAppId = () => {
    const appWindow = typeof window !== "undefined" ? window : undefined;
    if (appWindow?.__app_id)
        return appWindow.__app_id;
...
export const getMaterialInfo = (itemString) => {
    ...
    if (upperItem.includes("CST")) {
        return {
            ...
            icon: _jsx(Zap, { size: 12, className: "text-orange-600" }),
        };
    }
    if (upperItem.includes("EWT")) {
        return {
            ...
            icon: _jsx(Droplets, { size: 12, className: "text-cyan-600" }),
        };
    }
    return {
        type: "EST",
