...
export const generateLotNumber = (stationId, existingProducts = []) => {
    ...
    const count = existingProducts.filter((p) => p.lotNumber && p.lotNumber.toString().startsWith(prefix)).length;
...
export const getStationMachineCode = (stationId) => {
    ...
    if (stationToCode[normalized])
        return stationToCode[normalized];
