...
const detectMachineFromText = (value = "") => {
    ...
    if (machineMatch)
        return machineMatch[0].replace(/^40/, "");
    if (upper.includes("LOSSEN"))
        return "LOSSEN";
    if (upper.includes("NABEWERK"))
        return "NABEWERKING";
    if (upper.includes("BM01"))
        return "BM01";
    return "";
};
export const resolveInventoryScope = (record = {}) => {
    const departmentId = toSegment(record.departmentId || record.department || record.afdeling, DEFAULT_DEPARTMENT);
    const detectedMachine = record.machineId ||
        record.machine ||
        record.stationId ||
...
export const buildScopedInventoryDocPath = ({ docId, departmentId, machineId, }) => {
...
export const isProductionInventoryScopedDoc = (path = "") => String(path || "").includes("/future-factory/production/inventory/") &&
