...
const isFlangeText = (text) => {
    ...
    if (!normalizedText)
        return false;
    if (normalizedText.startsWith("FL ") || normalizedText === "FL")
        return true;
    if (normalizedText.includes(" FL "))
        return true;
    return normalizedText.includes("FLENS") || normalizedText.includes("FLANGE");
};
export const getFlangeSeriesInfo = (order, rawRules, rawToolingMolds = [], stationId = "", relatedItemCodes = []) => {
    ...
    const toolingCandidates = toolingMolds.filter((entry) => stationMatches(entry, stationId) && applicationMatchesFlange(entry));
    const matchedByItemCode = toolingCandidates.find((entry) => entry.itemCode && itemCodeCandidates.has(entry.itemCode));
    const matchedByMatcher = toolingCandidates.find((entry) => entry.matcher && matchesRule(searchableText, entry.matcher));
