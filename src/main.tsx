...
const isFirestoreUnexpectedStateError = (value: unknown): boolean => {
  const text = String(value || "").toLowerCase();
  return (
    text.includes("@firebase/firestore") &&
      text.includes("internal assertion failed") &&
      text.includes("unexpected state")
  ) || text.includes("firestore (10.14.1) internal assertion failed: unexpected state");
};

const reportRuntimeError = (type: string, details: unknown): void => {
  try {
    if (isFirestoreUnexpectedStateError(details)) {
      return;
    }
...
window.addEventListener('error', (event) => {
  const compactDetails = `${event.message || 'unknown'} @ ${event.filename || 'unknown'}:${event.lineno || 0}`;
  if (isFirestoreUnexpectedStateError(compactDetails)) {
    return;
  }
...
window.addEventListener('unhandledrejection', (event) => {
  ...
  if (isFirestoreUnexpectedStateError(message)) {
    return;
  }
