...
const toErrorMessage = (error) => {
    if (typeof error === "object" && error !== null && "message" in error) {
        return String(error.message || "Onbekende fout");
    }
...
const isActiveStatus = (status) => {
    if (!status)
        return true; // geen status = toon altijd
...
const resolveMachine = (data, fallbackId = "") => {
    const fromFields = data.currentMachineId ||
        data.machineId ||
...
export const getRawPlanningData = async (limitCount = 50) => {
    try {
        ...
        const snapshots = await Promise.all(paths.map((p) => getDocs(query(collection(db, getPathString(p)), limit(limitCount)))
            .catch(() => ({ docs: [] }))));
...
export const getTodayProductionContext = async () => {
    try {
        ...
        const trackSnap = await getDocs(query(collection(db, getPathString(trackingPath)), limit(800))).catch(() => ({ docs: [] }));
        ...
        trackSnap.docs.forEach((d) => {
            ...
            const startTs = toDateSafe(data.timestamps?.station_start ||
        ...
        const archSnap = await getDocs(query(collection(db, getPathString(archivePath)), limit(500))).catch(() => ({ docs: [] }));
        ...
        completedToday.forEach(({ orderId, lot, machine, item }) => {
            if (!byOrder.has(orderId))
                byOrder.set(orderId, { item, machine, lots: [] });
