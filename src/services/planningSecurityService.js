...
export const rejectTrackedProductFinal = async ({ productId, reasons = [], note = "", source = "", actorLabel = "", }) => {
    const payload = {
...
export const tempRejectTrackedProduct = async ({ productId, reasons = [], note = "", station = "", actorLabel = "", previousStep = "", previousStatus = "", source = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
        reasons: Array.isArray(reasons) ? reasons : [],
        note: String(note || "").trim(),
...
export const advanceTrackedProduct = async ({ productId, nextStation = "", nextStep, nextStatus, lastStation = "", note = "", actorLabel = "", previousStep = "", historyAction = "", historyDetails = "", clearManualMove = false, measurements = null, source = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
...
export const completeTrackedProductRepair = async ({ productId, station = "", actions = [], note = "", actorLabel = "", source = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
        station: String(station || "").trim(),
...
export const routeTrackedProductsToLossen = async ({ productIds, originStation = "", centralStation = "LOSSEN", centralOperators = [], actorLabel = "", source = "", }) => {
    const payload = {
...
export const startWorkstationProductionRun = async ({ orderDocId, lotStart, stringCount, stationId, orderDocPath = "", orderSourcePath = "", actorLabel = "", labelZplData = "", labelTemplateId = "", seriesGroupId = "", isFlangeSeries = false, stationOperators = [], source = "", }) => {
    const payload = {
...
export const toggleTrackedProductPause = async ({ productId, note = "", actorLabel = "", source = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
        note: String(note || "").trim(),
        actorLabel: String(actorLabel || "").trim(),
...
export const markTrackedProductReminder = async ({ productId, reminderSent = true, actorLabel = "", source = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
...
export const moveTrackedProductManual = async ({ productOrLotId, newStation, isRepairMove = false, repairInstruction = "", source = "", actorLabel = "", }) => {
    const payload = {
...
export const archiveRejectedTrackedProduct = async ({ productId, source = "", actorLabel = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
        source: String(source || "").trim(),
        actorLabel: String(actorLabel || "").trim(),
...
export const completeTrackedProduct = async ({ productId, finishType, fromStation = "", note = "", actorLabel = "", source = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
...
export const cancelTrackedProduction = async ({ productId, selectedStation = "", source = "", actorLabel = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
...
export const updatePlanningOrderPriority = async ({ orderDocId, priority, productDocId = "", source = "", actorLabel = "", }) => {
...
export const movePlanningOrder = async ({ orderDocId, targetType, targetId, currentDepartment = "", source = "", actorLabel = "", }) => {
    const payload = {
        orderDocId: String(orderDocId || "").trim(),
...
export const retrievePlanningOrder = async ({ orderDocId, source = "", actorLabel = "", }) => {
    const payload = {
        orderDocId: String(orderDocId || "").trim(),
        source: String(source || "").trim(),
        actorLabel: String(actorLabel || "").trim(),
...
export const togglePlanningOrderHold = async ({ orderDocId, source = "", actorLabel = "", }) => {
    const payload = {
        orderDocId: String(orderDocId || "").trim(),
        source: String(source || "").trim(),
        actorLabel: String(actorLabel || "").trim(),
...
export const updatePlanningOrderDetails = async ({ orderDocId, notes = "", plan = null, planDelta = null, started = null, source = "", actorLabel = "", }) => {
...
export const patchPlanningOrderMetadata = async ({ orderDocId, patch, source = "", actorLabel = "", }) => {
...
export const archivePlanningOrder = async ({ orderDocId, reason = "completed", source = "", actorLabel = "", }) => {
    const payload = {
        orderDocId: String(orderDocId || "").trim(),
...
export const assignOverproduction = async ({ targetOrderDocId, targetOrderId, productIds, routeStation, sourceOrderId = "", originMachine = "", source = "", actorLabel = "", }) => {
    const payload = {
...
export const cancelPlanningOrder = async ({ orderDocId, reason = "", source = "", actorLabel = "", }) => {
    const payload = {
        orderDocId: String(orderDocId || "").trim(),
...
export const assignPersonnelToStation = async ({ stationId, operatorId, operatorNumber = "", operatorName = "", date, departmentId = "", hoursWorked = 8, shiftType = "DAG", source = "", actorLabel = "", }) => {
    const payload = {
        stationId: String(stationId || "").trim(),
...
export const removePersonnelAssignment = async ({ assignmentId, stationId = "", source = "", actorLabel = "", }) => {
    const payload = {
...
export const loanPersonnelToDepartment = async ({ operatorNumber, operatorName = "", targetDepartment, targetStation, date, shiftLabel = "", shiftStart = "", shiftEnd = "", hoursWorked = 8, isPloeg = false, loanFromDepartment = "", loanFromStation = "", originalShift = "", source = "", actorLabel = "", }) => {
    const payload = {
        operatorNumber: String(operatorNumber || "").trim(),
        operatorName: String(operatorName || "").trim(),
...
export const saveOccupancyAssignments = async ({ records, source = "", actorLabel = "", }) => {
...
export const saveOccupancyAssignment = async ({ assignmentId, data, source = "", actorLabel = "", }) => {
...
export const deleteOccupancyAssignments = async ({ assignmentIds, source = "", actorLabel = "", }) => {
...
export const deleteOccupancyAssignment = async ({ assignmentId, source = "", actorLabel = "", }) => {
    const safeAssignmentId = String(assignmentId || "").trim();
    if (!safeAssignmentId) {
        throw new Error("assignmentId is verplicht.");
    }
...
export const savePersonnelRecord = async ({ personId = "", data, source = "", actorLabel = "", }) => {
...
export const createProductionMessages = async ({ messages, source = "", actorLabel = "", }) => {
    const payload = {
...
export const transitionPrintQueueJobStatus = async ({ jobId, status, error = "", source = "", actorLabel = "", }) => {
    const payload = {
...
export const requeuePrintQueueJob = async ({ jobId, source = "", actorLabel = "", }) => {
    const payload = {
        jobId: String(jobId || "").trim(),
        source: String(source || "").trim(),
        actorLabel: String(actorLabel || "").trim(),
...
export const deletePrintQueueJob = async ({ jobId, source = "", actorLabel = "", }) => {
    const payload = {
        jobId: String(jobId || "").trim(),
        source: String(source || "").trim(),
        actorLabel: String(actorLabel || "").trim(),
...
export const startProductionLots = async ({ orderDocId, orderDocPath = "", orderSourcePath = "", orderId, itemCode, item = "", lotStart, totalToProduce, stationId, stationLabel = "", actorLabel = "", labelZplData = "", labelTemplateId = "", seriesGroupId = "", isFlangeSeries = false, isVirtualLot = false, virtualReason = "", }) => {
    const payload = {
        orderDocId: String(orderDocId || "").trim(),
        orderDocPath: String(orderDocPath || "").trim(),
        orderSourcePath: String(orderSourcePath || "").trim(),
...
export const editTrackedProductLotNumber = async ({ productId, newLotNumber, reason, source = "", actorLabel = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
...
export const reassignTrackedProductOrder = async ({ productId, newOrderId, reason, source = "", actorLabel = "", }) => {
    const payload = {
        productId: String(productId || "").trim(),
...
export const linkPlanningOrderProduct = async ({ orderDocId, productId, productImage = "", }) => {
    const payload = {
        orderDocId: String(orderDocId || "").trim(),
        productId: String(productId || "").trim(),
...
export const createPlanningOrderManual = async ({ orderId, item, machine, plan, }) => {
    const payload = {
        orderId: String(orderId || "").trim(),
        item: String(item || "").trim(),
...
export const markMazakLabelsPrinted = async ({ productIds, stationId = "", isReprint = false, source = "", actorLabel = "", }) => {
    const payload = {
        productIds: Array.isArray(productIds) ? productIds.map((id) => String(id || "").trim()).filter(Boolean) : [],
        stationId: String(stationId || "").trim(),
...
export const appendQcNote = async ({ productId, note, archivedYear = null, source = "", actorLabel = "", }) => {
...
export const reserveAutoLotNumberRange = async ({ stationId, count = 1, reserve = true, }) => {
...
export const restoreArchivedTrackedProduct = async ({ productId, targetRoute, note = "", sourceContext = "TEAMLEADER_FULL_LIST", }) => {
    const safeProductId = String(productId || "").trim();
...
export const reportShopFloorIssue = async ({ type, machine = "", orderId = null, lotNumber = null, description = "", operatorName = "", }) => {
