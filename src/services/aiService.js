    ...
    async searchProductionOrders(searchTerm) {
        try {
            ...
            const filtered = orders.filter(order => {
                ...
                return (orderId.includes(term.toUpperCase()) ||
                    name.includes(term) ||
                    desc.includes(term) ||
            ...
            return filtered.sort((a, b) => {
                if (orderNumber) {
                    ...
                    if (aId === orderNumber && bId !== orderNumber)
                        return -1;
                    if (bId === orderNumber && aId !== orderNumber)
                        return 1;
                }
                return 0;
            });
...
- Spreiding voor te stellen (uren per dag) om deadline te halen\n`;
    ...
    extractSearchTerms(query) {
        ...
        const importantWords = words.filter(word => word.length >= 3 && !stopWords.includes(word));
    ...
    async searchCatalogProducts(searchTerm) {
        try {
            ...
            const term = searchTerm.toLowerCase();
            return products.filter(product => (product.name && product.name.toLowerCase().includes(term)) ||
    ...
    async getRelevantContext(userQuery) {
        try {
            ...
            try {
                ...
                console.warn('Kon geheugen niet laden voor context:', err);
            }
            ...
            if (docs.length > 0) {
                ...
                docs.slice(0, 3).forEach((docItem, idx) => {
                    ...
                    if (docItem.analysis?.title)
                        contextData += `Titel: ${docItem.analysis.title}\n`;
                    if (docItem.analysis?.summary)
                        contextData += `Samenvatting: ${docItem.analysis.summary}\n`;
                    ...
                    if (docItem.analysis?.keyFacts?.length)
                        contextData += `Kernpunten: ${docItem.analysis.keyFacts.join('; ')}\n`;
                    if (docItem.analysis?.processes?.length)
                        contextData += `Processen: ${docItem.analysis.processes.join('; ')}\n`;
                    if (docItem.analysis?.partNumbers?.length)
                        contextData += `Part Numbers: ${docItem.analysis.partNumbers.join(', ')}\n`;
                    if (docItem.analysis?.tolerances?.length)
                        contextData += `Toleranties: ${docItem.analysis.tolerances.join('; ')}\n`;
                    if (docItem.analysis?.warnings?.length)
                        contextData += `Waarschuwingen: ${docItem.analysis.warnings.join('; ')}\n`;
                    if (docItem.analysis?.tags?.length)
                        contextData += `Tags: ${docItem.analysis.tags.join(', ')}\n`;
            ...
            if (isOrderRelated || query.includes('product')) {
                ...
                if (orders.length > 0) {
                    ...
                    orders.slice(0, 3).forEach((order, idx) => {
                        ...
                        if (order.lotNumber)
                            contextData += `Lotnummer: ${order.lotNumber}\n`;
                        if (order.name)
                            contextData += `Product Naam: ${order.name}\n`;
                        if (order.sku)
                            contextData += `SKU: ${order.sku}\n`;
                        if (order.description)
                            contextData += `Beschrijving: ${order.description}\n`;
                        // Status informatie
                        if (order.status)
                            contextData += `Status: ${order.status}\n`;
                        if (order.workstation)
                            contextData += `Werkstation: ${order.workstation}\n`;
                        if (order.operator)
                            contextData += `Operator: ${order.operator}\n`;
                        ...
                        // Datum informatie
                        if (order.startDate)
                            contextData += `Startdatum: ${order.startDate}\n`;
                        if (order.dueDate)
                            contextData += `Vervaldatum: ${order.dueDate}\n`;
                        if (order.completedDate)
                            contextData += `Voltooid op: ${order.completedDate}\n`;
                        // Technische specs
                        if (order.specifications)
                            contextData += `Specificaties: ${JSON.stringify(order.specifications)}\n`;
                        if (order.tolerances)
                            contextData += `Toleranties: ${JSON.stringify(order.tolerances)}\n`;
                        // Kwaliteit/QC info
                        if (order.qualityStatus)
                            contextData += `Kwaliteit Status: ${order.qualityStatus}\n`;
                        if (order.defects)
                            contextData += `Geconstateerde afwijkingen: ${order.defects}\n`;
                        if (order.notes)
                            contextData += `Opmerkingen: ${order.notes}\n`;
                ...
                try {
                    ...
                    if (recent.length > 0) {
                        ...
                        recent.slice(0, 5).forEach((item, idx) => {
                            ...
                            if (lotNr)
                                contextData += `Lotnummer: ${lotNr}\n`;
                            const productName = item.name || item.productName || item.title || item.itemCode;
                            if (productName)
                                contextData += `Product: ${productName}\n`;
                            if (item.sku)
                                contextData += `SKU: ${item.sku}\n`;
                            ...
                            if (item.status)
                                contextData += `Status: ${item.status}\n`;
                            if (item.operator)
                                contextData += `Operator: ${item.operator}\n`;
                            if (item.workstation)
                                contextData += `Werkstation: ${item.workstation}\n`;
                        });
                        contextData += '\n' + '='.repeat(60) + '\n';
                    }
                }
                ...
                try {
                    ...
                    if (times.length > 0) {
                        ...
                        times.slice(0, 10).forEach((item, idx) => {
                            ...
                            const orderId = item.orderId || item.orderNumber || item.id || 'N/A';
                            contextData += `Order: ${orderId}\n`;
                            if (item.workstation || item.workcenter)
                                contextData += `Werkstation: ${item.workstation || item.workcenter}\n`;
                            if (item.operator)
                                contextData += `Operator: ${item.operator}\n`;
                            if (item.duration !== undefined)
                                contextData += `Duur: ${item.duration} min\n`;
                            if (item.setupTime !== undefined)
                                contextData += `Instelttijd: ${item.setupTime} min\n`;
                            if (item.cycleTime !== undefined)
                                contextData += `Cyclustijd: ${item.cycleTime} sec\n`;
                            if (item.totalTime !== undefined)
                                contextData += `Totale tijd: ${item.totalTime} min\n`;
                            if (item.startTime)
                                contextData += `Starttijd: ${item.startTime}\n`;
                            if (item.endTime)
                                contextData += `Eindtijd: ${item.endTime}\n`;
                            if (item.quantity !== undefined)
                                contextData += `Aantal: ${item.quantity}\n`;
                            if (item.efficiency !== undefined)
                                contextData += `Efficiëntie: ${item.efficiency}%\n`;
            ...
            const isCapacityQuery = query.includes('werkuur') || query.includes('werkuren') ||
            ...
            if (query.includes('maat') ||
                ...
                if (products.length > 0) {
                    ...
                    products.slice(0, 3).forEach((product, idx) => {
                        ...
                        if (product.sku)
                            contextData += `SKU: ${product.sku}\n`;
                        if (product.description)
                            contextData += `Beschrijving: ${product.description}\n`;
                        // Maten
                        if (product.diameter)
                            contextData += `Diameter: ${product.diameter}mm\n`;
                        if (product.length)
                            contextData += `Lengte: ${product.length}mm\n`;
                        if (product.width)
                            contextData += `Breedte: ${product.width}mm\n`;
                        if (product.height)
                            contextData += `Hoogte: ${product.height}mm\n`;
                        // Toleranties
                        if (product.tolerance)
                            contextData += `Tolerantie: ${JSON.stringify(product.tolerance)}\n`;
                        if (product.toleranceRange)
                            contextData += `Tolerantie Range: ${product.toleranceRange}\n`;
                        // Specificaties
                        ...
                        if (product.quantity !== undefined)
                            contextData += `Beschikbare Hoeveelheid: ${product.quantity}\n`;
                        if (product.minStock)
                            contextData += `Min. Voorraad: ${product.minStock}\n`;
                        if (product.reserved)
                            contextData += `Gereserveerd: ${product.reserved}\n`;
                    });
                    contextData += '\n' + '='.repeat(60) + '\n';
                }
            }
    ...
    async saveMemory({ topic, content, sourceQuestion = "", sourceAnswer = "", userId = null, category = "approved_answer" }) {
        try {
            ...
            await logActivity(userId || 'system', 'AI_MEMORY_SAVE', `AI memory opgeslagen: ${topic || 'zonder onderwerp'}`);
    ...
    async saveConversation({ userId, sessionId, messages }) {
        ...
        try {
            ...
            await logActivity(userId, 'AI_CONVERSATION_SAVE', `AI conversatie opgeslagen (${toSave.length} berichten)`);
        }
        catch (error) {
